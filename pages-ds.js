...
${quizHTML('ds-arrays', [
  { q: "Time complexity of ArrayList.get(i) vs LinkedList.get(i)?", opts: ["Both O(n)", "ArrayList O(1), LinkedList O(n)", "ArrayList O(n), LinkedList O(1)", "Both O(1)"], ans: 1, exp: "ArrayList backed by an array — direct index access O(1). LinkedList must traverse from head/tail to index i — O(n). This is why ArrayList dominates for random access patterns." },
  ...
  { q: "Two Sum problem — optimal solution?", opts: ["Nested loops O(n²)", "HashMap: store complement, one pass O(n)", "Binary search O(n log n)", "Sort then two pointers O(n log n)"], ans: 1, exp: "HashMap approach: for each num, check if (target - num) is in map. If yes, found pair. If no, add num to map. Single pass O(n) time, O(n) space. This is the expected O(n) solution." },
  { q: "StringBuilder vs String concatenation in a loop — why does it matter?", opts: ["String is immutable — each + creates a new object. StringBuilder is mutable — O(n) total vs O(n²)", "StringBuilder is thread-safe", "No difference", "String + is faster for small strings"], ans: 0, exp: "String s = ''; for n iterations: s += x creates n new String objects (O(n²) total). StringBuilder.append() amortizes to O(1) per append, O(n) total. Never concatenate strings in loops." },
  { q: "Sliding window for 'max sum of k consecutive elements' — complexity?", opts: ["O(n) — one pass with a sliding window of size k", "O(n log k)", "O(k)", "O(n²)"], ans: 0, exp: "Sliding window: compute first window sum, then slide right adding new element and removing leftmost — O(n). Brute force recomputing each window is O(nk). This pattern generalises to many substring/subarray problems." },
  ...
  { q: "Dutch National Flag algorithm solves?", opts: ["Anagram detection", "Reverse linked list", "Sort array of 0s, 1s, 2s in O(n) time O(1) space (3-way partition)", "Detect cycle"], ans: 2, exp: "Three pointers: low=0, mid=0, high=n-1. If nums[mid]==0: swap with low, advance both. If 1: advance mid. If 2: swap with high, retreat high. Single pass O(n), in-place. Used in QuickSort 3-way partition." },
  { q: "What does Character.isDigit('5') return in Java?", opts: ["'5' (char)", "true", "false", "5 (int)"], ans: 1, exp: "Character.isDigit(char c) returns boolean true for '0'–'9'. Similarly: isLetter(), isUpperCase(), isLowerCase(), toUpperCase(), toLowerCase(). These are the essential String manipulation utilities for interview coding." },
  { q: "Best approach to check if a string is a palindrome?", opts: ["HashMap of characters", "Reverse string and compare O(n) extra space", "Two pointers from both ends, O(n) time O(1) space", "Stack-based comparison"], ans: 2, exp: "Two pointers: left=0, right=len-1. While left<right: if chars differ, not palindrome. O(n) time, O(1) space. Reversing uses O(n) extra space. For interview, always prefer the two-pointer approach." }
])}
...
${quizHTML('ds-linkedlist', [
  { q: "How to detect a cycle in a linked list — optimal?", opts: ["Use a HashSet of visited nodes O(n) space", "Reverse the list", "Count nodes and compare to expected", "Floyd's slow/fast pointer — O(n) time O(1) space"], ans: 3, exp: "Floyd's cycle detection: slow moves 1 step, fast moves 2 steps. If there's a cycle, fast eventually laps slow and they meet. O(n) time, O(1) space — much better than HashSet's O(n) space." },
  { q: "Reverse a linked list iteratively — what 3 pointers do you need?", opts: ["prev, current, next — prev starts null", "left, right, mid", "head, tail, current", "start, end, temp"], ans: 0, exp: "prev=null, curr=head. Loop: save next=curr.next, point curr.next=prev, advance prev=curr, curr=next. When curr==null, prev is new head. Classic O(n) time O(1) space pattern." },
  { q: "Find the middle of a linked list — best approach?", opts: ["Convert to array, find middle", "Reverse and count", "Count nodes, traverse to n/2", "Slow/fast pointer: slow moves 1, fast moves 2 — when fast reaches end, slow is at middle"], ans: 3, exp: "Slow/fast pointers: fast moves 2x speed. When fast reaches null (or last node for odd length), slow is at the middle. O(n) time, O(1) space, single pass. Used in merge sort on linked lists." },
  { q: "LinkedList as Deque in Java — which operations are O(1)?", opts: ["addFirst, addLast, removeFirst, removeLast, peekFirst, peekLast — all O(1)", "Only addFirst and addLast", "All operations except remove", "Only remove operations"], ans: 0, exp: "Java's LinkedList implements Deque. Since it's doubly linked with head/tail pointers, all head and tail operations are O(1). Use ArrayDeque instead — same O(1) ops but no object overhead (faster in practice)." },
  { q: "Merge two sorted linked lists — approach?", opts: ["Sort the combined list", "Use priority queue", "Convert both to arrays, merge, convert back", "Iterative comparison with dummy head node — O(m+n) time O(1) space"], ans: 3, exp: "Dummy head trick: create dummy node, current pointer. Compare heads of both lists, attach smaller to current, advance. When one list exhausted, attach rest of other. O(m+n) time, O(1) space — no extra memory." },
  { q: "What is the advantage of a doubly linked list over singly?", opts: ["Uses less memory", "O(1) removal of any node (if you have node reference) and backward traversal", "Supports sorting", "Faster insertion at head"], ans: 1, exp: "Singly: to delete a node, you need its predecessor (O(n) to find). Doubly: node.prev.next = node.next in O(1). Also enables backward traversal (for LRU, deque operations). Cost: 2× pointer memory per node." },
  { q: "Reverse linked list in groups of k — key challenge?", opts: ["Only works for even k", "Need to know total length first", "No special challenge", "Must track the tail of each reversed group to connect to next group's head"], ans: 3, exp: "After reversing a group of k: the original head becomes the tail. Connect it to the head of the next reversed group. Tricky: save the connection point before reversing. Common in LeetCode hard problems." },
  { q: "Java Stack vs Deque — which to use?", opts: ["LinkedList only", "Deque (ArrayDeque) — Stack is a legacy class with synchronisation overhead", "Both are identical", "Stack (legacy class)"], ans: 1, exp: "Stack extends Vector which is synchronised — slow. ArrayDeque implements Deque with push/pop O(1) and no sync overhead. Java docs explicitly recommend ArrayDeque over Stack for stack use cases." },
  { q: "Intersection of two linked lists — find the meeting node?", opts: ["Advance longer list by length difference, then traverse together until they meet — O(m+n) O(1)", "Use sorting", "Use two HashSets", "Reverse both lists"], ans: 0, exp: "1) Find lengths of both lists. 2) Advance the pointer on the longer list by (lenA - lenB) steps. 3) Move both pointers together until they point to the same node (same reference, not just equal value). O(m+n) time, O(1) space." }
])}
...
${quizHTML('ds-hashmap', [
  ...
  { q: "Why does HashMap rehash when load factor exceeds 0.75?", opts: ["To keep average bucket chain length short — O(1) average lookup degrades as load increases", "To support thread safety", "To save memory", "Required by Java spec"], ans: 0, exp: "Load factor = entries / buckets. At 0.75, average chain length is manageable. Above 0.75, more collisions → longer chains → O(n) lookups. Rehashing doubles capacity and redistributes all entries. Default 0.75 balances memory vs performance." },
  { q: "ConcurrentHashMap vs synchronized HashMap — why is ConcurrentHashMap faster?", opts: ["Segment/bucket-level locking: reads are lock-free, writes lock only affected bucket", "ConcurrentHashMap uses no locks", "synchronized HashMap uses CAS", "ConcurrentHashMap is single-threaded"], ans: 0, exp: "Java 8 ConcurrentHashMap: reads use volatile (lock-free), writes use CAS + synchronized on individual bucket. Synchronized HashMap wraps every method with one lock — all threads blocked for all operations." },
  { q: "LinkedHashMap for LRU — what constructor flag is needed?", opts: ["LinkedHashMap doesn't support LRU", "new LinkedHashMap(cap, 0.75f, true) — accessOrder=true", "new LinkedHashMap(cap, 0.75f, false)", "new LinkedHashMap(cap)"], ans: 1, exp: "The third boolean param is accessOrder. false (default) = insertion order. true = access order — every get()/put() moves the entry to the tail. Override removeEldestEntry() to evict when size exceeds capacity." },
  { q: "HashMap get() average vs worst case?", opts: ["O(n) always", "O(1) average, O(log n) worst case (tree bucket, Java 8+)", "O(log n) average, O(n) worst", "O(1) average, O(1) worst"], ans: 1, exp: "With good hash distribution: O(1). Worst case pre-Java8: all keys hash to same bucket → O(n). Java 8+: long chains convert to Red-Black trees → O(log n) worst case. Deliberate hash collision attacks now have bounded impact." },
  { q: "Can HashMap have null keys? Can Hashtable?", opts: ["Neither supports null", "Both support multiple null keys", "Only Hashtable supports null keys", "HashMap: one null key allowed. Hashtable: no null keys (throws NullPointerException)"], ans: 3, exp: "HashMap handles null key specially — it's stored in bucket 0, bypassing the hash function. Hashtable is legacy, thread-safe, but throws NPE on null keys/values. ConcurrentHashMap also does NOT allow null keys or values." },
  { q: "What happens when you use a mutable object as a HashMap key?", opts: ["HashMap makes an immutable copy", "If the key's hashCode changes after insertion, the entry becomes unreachable (lost in wrong bucket)", "It works fine always", "HashMap updates the bucket automatically"], ans: 1, exp: "HashMap uses hashCode() to find the bucket. If you modify the key object after put(), its hashCode may change → HashMap looks in the wrong bucket → entry is 'lost'. Always use immutable keys (String, Integer, UUID) or final fields in hashCode." },
  { q: "TreeMap vs HashMap — when do you choose TreeMap?", opts: ["TreeMap when you need sorted key order, range queries, or floor/ceiling operations — O(log n) vs HashMap O(1)", "TreeMap is always faster", "TreeMap uses less memory", "HashMap doesn't support get()"], ans: 0, exp: "TreeMap (Red-Black tree): keys always sorted, supports floorKey(), ceilingKey(), subMap(), headMap(), tailMap() — perfect for range queries. O(log n) all ops. HashMap O(1) but unordered. Use TreeMap when order matters." },
  { q: "How to count word frequencies in Java using Map?", opts: ["Sort array first", "Use indexOf on each word", "Use a List and sort", "map.merge(word, 1, Integer::sum) or getOrDefault pattern"], ans: 3, exp: "Clean idiom: map.merge(word, 1, Integer::sum) — if key absent, puts 1; if present, applies the merging function (adds 1). Alternative: map.put(word, map.getOrDefault(word, 0) + 1). merge() is cleaner and more readable." }
])}
...
${quizHTML('ds-trees', [
  { q: "Inorder traversal of a BST gives?", opts: ["Elements in sorted ascending order", "Level-by-level order", "Elements in reverse sorted order", "Random order"], ans: 0, exp: "BST property: left < root < right. Inorder (left → root → right) visits nodes in ascending sorted order. This is why 'validate BST' and 'kth smallest in BST' both use inorder traversal." },
  ...
  { q: "Height of a balanced binary tree with n nodes?", opts: ["O(1)", "O(log n)", "O(n²)", "O(n)"], ans: 1, exp: "A balanced binary tree has O(log n) height. Each level doubles node count: level 0=1, level 1=2, level 2=4 → height = log₂(n). Unbalanced (degenerate/linked-list shape) → O(n) height. This is why AVL/Red-Black trees are important." },
  { q: "What is a Trie best used for?", opts: ["Sorting integers", "Graph traversal", "Balanced sorting", "Prefix-based search (autocomplete, spell check) — O(m) lookup where m = word length"], ans: 3, exp: "Trie (prefix tree): each node represents a character. Common prefixes share nodes. insert/search/startsWith all O(m) where m=word length — independent of dictionary size. Used in autocomplete, spell checkers, IP routing (prefix matching)." },
  { q: "Lowest Common Ancestor (LCA) of two nodes — approach?", opts: ["Recursive: if root is null or equals either node, return root; recurse both sides; if both non-null, root is LCA", "Use inorder traversal", "Sort nodes by value", "BFS from both nodes"], ans: 0, exp: "Recursive LCA: search left and right subtrees. If both return non-null → current node is LCA (nodes are on different sides). If only one returns non-null → both nodes in same subtree (return that result). O(n) time." },
  { q: "Java TreeMap vs TreeSet — difference?", opts: ["TreeMap: sorted key-value pairs. TreeSet: sorted unique keys only (backed by TreeMap with dummy values)", "TreeMap doesn't sort", "TreeSet is faster", "No difference"], ans: 0, exp: "Both backed by Red-Black tree, O(log n) ops. TreeMap: key→value store with sorted keys, supports floorEntry(), ceilingEntry(), subMap(). TreeSet: unique sorted elements, supports floor(), ceiling(), subset(). TreeSet is TreeMap<K, Boolean>." },
  { q: "Max depth vs diameter of a binary tree?", opts: ["Max depth: longest root-to-leaf path. Diameter: longest path between any two nodes (may not pass through root)", "Diameter is always twice max depth", "They are the same", "Diameter counts edges not nodes"], ans: 0, exp: "Max depth: longest root-to-leaf chain. Diameter: longest path between ANY two nodes — could go through left subtree, through root, into right subtree. Diameter = max(left_height + right_height) for each node. Both solved with DFS." },
  { q: "Heap vs BST — which to prefer for 'find minimum'?", opts: ["They are equivalent", "Depends on language", "Min-heap: O(1) peek minimum. BST: O(log n) to find leftmost. Heap wins for top-K and priority queue", "BST always"], ans: 2, exp: "Min-heap peek(): O(1) — root is always minimum. Insertion/removal: O(log n). BST minimum: O(log n) traverse left. For priority queue, repeated find-min: heap is better. For range queries/sorted iteration: BST is better." },
  { q: "Serialize and deserialize a binary tree — which traversal?", opts: ["Level-order only", "Inorder alone (ambiguous)", "Postorder only", "Preorder (root-left-right) with null markers — uniquely identifies tree structure"], ans: 3, exp: "Inorder alone is ambiguous (multiple trees can have same inorder). Preorder with null markers ('1,2,null,null,3,null,null') uniquely encodes structure. During deserialization, split by commas and recursively build tree. Level-order (BFS) also works." }
])}
...
${quizHTML('ds-graph', [
  ...
  { q: "Topological sort — what graph property is required?", opts: ["Graph must be undirected", "Graph must have equal in/out degrees", "Graph must be a DAG (Directed Acyclic Graph)", "Graph must be connected"], ans: 2, exp: "Topological sort only works on DAGs. Ordering: for every edge u→v, u appears before v. Used for: build dependency order, course prerequisites, task scheduling. Two algorithms: DFS (reverse finish order) or Kahn's (BFS with in-degree tracking)." },
  { q: "Number of islands problem — best approach?", opts: ["Dynamic programming", "Sort the grid", "Union-Find only", "DFS/BFS flood fill: for each unvisited '1', run DFS marking all connected '1's as visited, increment count"], ans: 3, exp: "Flood fill DFS: iterate grid, when '1' found, do DFS to mark entire island as visited ('0' or separate visited array), increment count. O(m×n) time. Union-Find is an alternative — connect adjacent '1's, count components." },
  { q: "Dijkstra's algorithm fails when?", opts: ["Graph is disconnected", "Graph has negative edge weights", "Graph has more than 1000 nodes", "Graph is undirected"], ans: 1, exp: "Dijkstra's greedy assumption: once a node is finalized, its distance is correct. With negative edges, a later path could improve an already-finalized distance. Use Bellman-Ford for negative weights (O(VE)) or Floyd-Warshall for all-pairs." },
...
${quizHTML('ds-heap', [
  { q: "Min-heap vs max-heap — which is used for 'find K largest elements'?", opts: ["Max-heap of size N", "Sorted array", "Min-heap of size K — smallest of the K largest is at root, compare each new element against root", "Binary search tree"], ans: 2, exp: "Keep a min-heap of size K. For each element: if larger than root, remove root and insert element. At end, heap contains K largest elements. Root = Kth largest. O(N log K) time, O(K) space." },
...
${quizHTML('algo-patterns', [
  { q: "Which pattern to use when 'find all subsets/permutations'?", opts: ["Binary search", "Two pointers", "Backtracking — explore all choices recursively, undo each choice before trying next", "Sliding window"], ans: 2, exp: "Backtracking = systematic brute force with pruning. Build solution incrementally, abandon paths that can't lead to valid solution. Subsets, permutations, N-Queens, Sudoku all use this pattern." },
  { q: "Sliding window vs two pointers — key difference?", opts: ["They're identical", "Sliding window is always O(N²)", "Two pointers only works on sorted arrays", "Sliding window maintains a contiguous range and expands/shrinks it. Two pointers can move independently from both ends."], ans: 3, exp: "Sliding window: contiguous subarray/substring, expand right, shrink left when constraint violated. Two pointers: often start from both ends (sorted array) or fast/slow (linked list). Different problems, different patterns." }
])}
...
${quizHTML('algo-twopointer', [
  ...
  { q: "3Sum — how to avoid duplicate triplets?", opts: ["Use a HashSet for all triplets", "Sort array, skip duplicate values at each pointer position after finding a valid triplet", "Sort by absolute value", "Use HashMap to count frequencies"], ans: 1, exp: "After sorting: outer loop skips if nums[i] == nums[i-1]. When triplet found, skip duplicates for both left and right (while nums[left]==nums[left+1]: left++). O(n^2) total with deduplication handled inline." },
  { q: "Slow/fast pointer: after finding cycle, how to find cycle START?", opts: ["Use a HashSet", "Keep fast at meeting point", "Reset slow to head, advance both one step at a time — meeting point is cycle start", "Count nodes in cycle"], ans: 2, exp: "After detection, reset slow to head. Advance both pointers 1 step at a time. They meet exactly at the cycle start. Mathematical proof: distance head-to-start equals distance meeting-point-to-start within the cycle." },
  ...
  { q: "Remove duplicates from sorted array in-place?", opts: ["Sort first", "Count occurrences first", "Slow pointer tracks last unique; fast pointer finds next unique — copy to slow+1", "Use a set"], ans: 2, exp: "Slow=0 (last written), fast scans. When nums[fast] != nums[slow]: slow++, copy nums[fast] to nums[slow]. Return slow+1 as count of unique elements. In-place O(n) time O(1) space." },
  { q: "Merge two sorted arrays in-place — which direction?", opts: ["Doesn't matter", "Right to left — start from the end to avoid overwriting elements not yet processed", "Sort after copying", "Left to right"], ans: 1, exp: "Merging left-to-right into array A overwrites elements not yet compared. Merge right-to-left: start from end of both, place larger at end of A. No overwrites possible. O(m+n) time O(1) space." },
  { q: "Valid palindrome — two pointer handling of non-alphanumeric chars?", opts: ["Always return false if non-alnum present", "Remove all non-alnum first O(n) space", "Convert to lowercase first", "Two pointers skip non-alphanumeric inline — O(1) extra space"], ans: 3, exp: "While left < right: if not alphanumeric, skip that pointer. Else compare toLowerCase. Skip-inline approach: O(n) time O(1) space — no extra string creation. This is the canonical O(1) space palindrome check." }
])}
`;
...
${quizHTML('algo-sliding', [
  { q: "When to use sliding window?", opts: ["Only for sorted arrays", "When finding optimal contiguous subarray/substring — max sum of size K, longest substring without repeating, etc.", "When elements need sorting", "Any array problem"], ans: 1, exp: "Sliding window keywords: 'contiguous', 'subarray', 'substring', 'window of size K', 'longest/shortest with condition'. Fixed-size window: move both pointers. Variable-size: expand right, shrink left when constraint breaks." },
  { q: "Time complexity of sliding window?", opts: ["O(2^N)", "O(N) — each element is added and removed at most once", "O(N log N)", "O(N²)"], ans: 1, exp: "Despite having a while loop inside the for loop, each element enters the window once (right pointer) and leaves once (left pointer). Total operations = 2N = O(N). This is the key insight interviewers look for." }
])}
...
${quizHTML('algo-binsearch', [
  { q: "Binary search on answer space — what does that mean?", opts: ["Searching a sorted array", "The answer itself is monotonic — binary search on possible answer values, check if each candidate is feasible", "Using two binary searches", "Searching a 2D matrix"], ans: 1, exp: "Example: 'minimum capacity to ship packages in D days'. Answer range: max(weights) to sum(weights). Binary search on capacity value, for each candidate check if it's feasible (can ship in ≤D days). Works when: answer is monotonic (if X works, X+1 also works)." },
  { q: "lo < hi vs lo <= hi — when to use which?", opts: ["Always use lo <= hi", "lo < hi is faster", "No difference", "lo < hi when converging to a single point (search space shrinks to 1). lo <= hi when searching for exact match."], ans: 3, exp: "lo <= hi: classic search, return mid when found. lo < hi: find insertion point or boundary — loop ends when lo == hi == answer. Using wrong one causes infinite loops or off-by-one errors. Practice both patterns." }
])}
...
${quizHTML('algo-bfsdfs', [
  { q: "BFS vs DFS — when to prefer BFS?", opts: ["Always use BFS", "When finding shortest path in unweighted graph — BFS guarantees shortest path, DFS does not", "BFS uses less memory", "DFS can't handle cycles"], ans: 1, exp: "BFS explores level by level → first time you reach a node is the shortest path (in unweighted graphs). DFS may find a longer path first. BFS uses O(V) memory (queue), DFS uses O(depth) stack. Use DFS for: exhaustive search, backtracking, detecting cycles." },
  { q: "How to detect a cycle in a directed graph?", opts: ["DFS with 3 states: unvisited, in-progress (on current path), visited. Back edge to in-progress node = cycle", "Check if edge count > vertex count", "BFS only", "Topological sort only"], ans: 0, exp: "Color each node: WHITE (unvisited), GRAY (in current DFS path), BLACK (fully processed). If DFS encounters a GRAY node, there's a cycle on the current path. This is more precise than just 'visited' — it distinguishes back edges from cross edges." }
])}
...
${quizHTML('algo-backtrack', [
  { q: "Backtracking template — what are the 3 key steps?", opts: ["Choose (add to path) → Explore (recurse) → Unchoose (remove from path, try next option)", "Divide, conquer, combine", "Sort, search, return", "Hash, compare, merge"], ans: 0, exp: "Every backtracking problem: 1) Choose: add element to current solution. 2) Explore: recurse with remaining choices. 3) Unchoose: remove element, restore state, try next option. The 'unchoose' step is what distinguishes backtracking from plain recursion." },
...
${quizHTML('algo-topk', [
  { q: "Top-K elements — heap vs quickselect?", opts: ["Quickselect is always better", "They give different results", "Heap: O(N log K) guaranteed. Quickselect: O(N) average but O(N²) worst case. Heap for streaming data, quickselect for in-memory.", "Always use sorting"], ans: 2, exp: "Min-heap of size K: process each element O(log K), total O(N log K), works with streaming data. Quickselect (modified quicksort): partition around pivot, recurse only on the side containing Kth element. O(N) average, but worst case O(N²) without random pivot." }
])}
