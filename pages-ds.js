...
${quizHTML('ds-arrays', [
  { q: "Time complexity of ArrayList.get(i) vs LinkedList.get(i)?", opts: ["Both O(n)", "ArrayList O(1), LinkedList O(n)", "Both O(1)", "ArrayList O(n), LinkedList O(1)"], ans: 1, exp: "ArrayList backed by an array — direct index access O(1). LinkedList must traverse from head/tail to index i — O(n). This is why ArrayList dominates for random access patterns." },
  ...
  { q: "Two Sum problem — optimal solution?", opts: ["HashMap: store complement, one pass O(n)", "Binary search O(n log n)", "Sort then two pointers O(n log n)", "Nested loops O(n²)"], ans: 0, exp: "HashMap approach: for each num, check if (target - num) is in map. If yes, found pair. If no, add num to map. Single pass O(n) time, O(n) space. This is the expected O(n) solution." },
  { q: "StringBuilder vs String concatenation in a loop — why does it matter?", opts: ["StringBuilder is thread-safe", "String is immutable — each + creates a new object. StringBuilder is mutable — O(n) total vs O(n²)", "String + is faster for small strings", "No difference"], ans: 1, exp: "String s = ''; for n iterations: s += x creates n new String objects (O(n²) total). StringBuilder.append() amortizes to O(1) per append, O(n) total. Never concatenate strings in loops." },
  { q: "Sliding window for 'max sum of k consecutive elements' — complexity?", opts: ["O(k)", "O(n²)", "O(n) — one pass with a sliding window of size k", "O(n log k)"], ans: 2, exp: "Sliding window: compute first window sum, then slide right adding new element and removing leftmost — O(n). Brute force recomputing each window is O(nk). This pattern generalises to many substring/subarray problems." },
  { q: "Find first and last occurrence of target in sorted array — optimal?", opts: ["Sort then scan", "Two binary searches O(log n) — one for first, one for last", "Hash map", "Linear scan O(n)"], ans: 1, exp: "Two binary searches: first finds leftmost occurrence (if nums[mid]==target, update result and continue left). Second finds rightmost (continue right). Both O(log n). Classic binary search variant." },
  { q: "Dutch National Flag algorithm solves?", opts: ["Detect cycle", "Sort array of 0s, 1s, 2s in O(n) time O(1) space (3-way partition)", "Reverse linked list", "Anagram detection"], ans: 1, exp: "Three pointers: low=0, mid=0, high=n-1. If nums[mid]==0: swap with low, advance both. If 1: advance mid. If 2: swap with high, retreat high. Single pass O(n), in-place. Used in QuickSort 3-way partition." },
  { q: "What does Character.isDigit('5') return in Java?", opts: ["true", "'5' (char)", "5 (int)", "false"], ans: 0, exp: "Character.isDigit(char c) returns boolean true for '0'–'9'. Similarly: isLetter(), isUpperCase(), isLowerCase(), toUpperCase(), toLowerCase(). These are the essential String manipulation utilities for interview coding." },
  { q: "Best approach to check if a string is a palindrome?", opts: ["Stack-based comparison", "Reverse string and compare O(n) extra space", "HashMap of characters", "Two pointers from both ends, O(n) time O(1) space"], ans: 3, exp: "Two pointers: left=0, right=len-1. While left<right: if chars differ, not palindrome. O(n) time, O(1) space. Reversing uses O(n) extra space. For interview, always prefer the two-pointer approach." }
])}
...
${quizHTML('ds-linkedlist', [
  { q: "How to detect a cycle in a linked list — optimal?", opts: ["Reverse the list", "Floyd's slow/fast pointer — O(n) time O(1) space", "Count nodes and compare to expected", "Use a HashSet of visited nodes O(n) space"], ans: 1, exp: "Floyd's cycle detection: slow moves 1 step, fast moves 2 steps. If there's a cycle, fast eventually laps slow and they meet. O(n) time, O(1) space — much better than HashSet's O(n) space." },
  { q: "Reverse a linked list iteratively — what 3 pointers do you need?", opts: ["head, tail, current", "prev, current, next — prev starts null", "start, end, temp", "left, right, mid"], ans: 1, exp: "prev=null, curr=head. Loop: save next=curr.next, point curr.next=prev, advance prev=curr, curr=next. When curr==null, prev is new head. Classic O(n) time O(1) space pattern." },
  { q: "Find the middle of a linked list — best approach?", opts: ["Slow/fast pointer: slow moves 1, fast moves 2 — when fast reaches end, slow is at middle", "Convert to array, find middle", "Reverse and count", "Count nodes, traverse to n/2"], ans: 0, exp: "Slow/fast pointers: fast moves 2x speed. When fast reaches null (or last node for odd length), slow is at the middle. O(n) time, O(1) space, single pass. Used in merge sort on linked lists." },
  { q: "LinkedList as Deque in Java — which operations are O(1)?", opts: ["All operations except remove", "Only remove operations", "addFirst, addLast, removeFirst, removeLast, peekFirst, peekLast — all O(1)", "Only addFirst and addLast"], ans: 2, exp: "Java's LinkedList implements Deque. Since it's doubly linked with head/tail pointers, all head and tail operations are O(1). Use ArrayDeque instead — same O(1) ops but no object overhead (faster in practice)." },
  { q: "Merge two sorted linked lists — approach?", opts: ["Use priority queue", "Convert both to arrays, merge, convert back", "Sort the combined list", "Iterative comparison with dummy head node — O(m+n) time O(1) space"], ans: 3, exp: "Dummy head trick: create dummy node, current pointer. Compare heads of both lists, attach smaller to current, advance. When one list exhausted, attach rest of other. O(m+n) time, O(1) space — no extra memory." },
  { q: "What is the advantage of a doubly linked list over singly?", opts: ["Supports sorting", "Faster insertion at head", "O(1) removal of any node (if you have node reference) and backward traversal", "Uses less memory"], ans: 2, exp: "Singly: to delete a node, you need its predecessor (O(n) to find). Doubly: node.prev.next = node.next in O(1). Also enables backward traversal (for LRU, deque operations). Cost: 2× pointer memory per node." },
  { q: "Reverse linked list in groups of k — key challenge?", opts: ["No special challenge", "Only works for even k", "Must track the tail of each reversed group to connect to next group's head", "Need to know total length first"], ans: 2, exp: "After reversing a group of k: the original head becomes the tail. Connect it to the head of the next reversed group. Tricky: save the connection point before reversing. Common in LeetCode hard problems." },
  { q: "Java Stack vs Deque — which to use?", opts: ["Both are identical", "LinkedList only", "Stack (legacy class)", "Deque (ArrayDeque) — Stack is a legacy class with synchronisation overhead"], ans: 3, exp: "Stack extends Vector which is synchronised — slow. ArrayDeque implements Deque with push/pop O(1) and no sync overhead. Java docs explicitly recommend ArrayDeque over Stack for stack use cases." },
  { q: "Intersection of two linked lists — find the meeting node?", opts: ["Use two HashSets", "Advance longer list by length difference, then traverse together until they meet — O(m+n) O(1)", "Use sorting", "Reverse both lists"], ans: 1, exp: "1) Find lengths of both lists. 2) Advance the pointer on the longer list by (lenA - lenB) steps. 3) Move both pointers together until they point to the same node (same reference, not just equal value). O(m+n) time, O(1) space." }
])}
...
${quizHTML('ds-hashmap', [
  { q: "How does HashMap resolve collisions in Java 8+?", opts: ["Cuckoo hashing", "Double hashing", "Open addressing / linear probing", "Separate chaining: linked list, converts to balanced tree (Red-Black) when bucket size > 8"], ans: 3, exp: "Java 8 HashMap: each bucket starts as a linked list. When a bucket has >8 entries, it converts to a TreeMap (Red-Black tree) — O(log n) worst case vs O(n) for a long chain. Converts back to list when size drops to 6." },
  { q: "Why does HashMap rehash when load factor exceeds 0.75?", opts: ["To save memory", "To support thread safety", "Required by Java spec", "To keep average bucket chain length short — O(1) average lookup degrades as load increases"], ans: 3, exp: "Load factor = entries / buckets. At 0.75, average chain length is manageable. Above 0.75, more collisions → longer chains → O(n) lookups. Rehashing doubles capacity and redistributes all entries. Default 0.75 balances memory vs performance." },
  { q: "ConcurrentHashMap vs synchronized HashMap — why is ConcurrentHashMap faster?", opts: ["Segment/bucket-level locking: reads are lock-free, writes lock only affected bucket", "synchronized HashMap uses CAS", "ConcurrentHashMap is single-threaded", "ConcurrentHashMap uses no locks"], ans: 0, exp: "Java 8 ConcurrentHashMap: reads use volatile (lock-free), writes use CAS + synchronized on individual bucket. Synchronized HashMap wraps every method with one lock — all threads blocked for all operations." },
  { q: "LinkedHashMap for LRU — what constructor flag is needed?", opts: ["new LinkedHashMap(cap)", "LinkedHashMap doesn't support LRU", "new LinkedHashMap(cap, 0.75f, true) — accessOrder=true", "new LinkedHashMap(cap, 0.75f, false)"], ans: 2, exp: "The third boolean param is accessOrder. false (default) = insertion order. true = access order — every get()/put() moves the entry to the tail. Override removeEldestEntry() to evict when size exceeds capacity." },
  { q: "HashMap get() average vs worst case?", opts: ["O(1) average, O(1) worst", "O(n) always", "O(1) average, O(log n) worst case (tree bucket, Java 8+)", "O(log n) average, O(n) worst"], ans: 2, exp: "With good hash distribution: O(1). Worst case pre-Java8: all keys hash to same bucket → O(n). Java 8+: long chains convert to Red-Black trees → O(log n) worst case. Deliberate hash collision attacks now have bounded impact." },
  { q: "Can HashMap have null keys? Can Hashtable?", opts: ["HashMap: one null key allowed. Hashtable: no null keys (throws NullPointerException)", "Neither supports null", "Both support multiple null keys", "Only Hashtable supports null keys"], ans: 0, exp: "HashMap handles null key specially — it's stored in bucket 0, bypassing the hash function. Hashtable is legacy, thread-safe, but throws NPE on null keys/values. ConcurrentHashMap also does NOT allow null keys or values." },
  { q: "What happens when you use a mutable object as a HashMap key?", opts: ["If the key's hashCode changes after insertion, the entry becomes unreachable (lost in wrong bucket)", "HashMap makes an immutable copy", "It works fine always", "HashMap updates the bucket automatically"], ans: 0, exp: "HashMap uses hashCode() to find the bucket. If you modify the key object after put(), its hashCode may change → HashMap looks in the wrong bucket → entry is 'lost'. Always use immutable keys (String, Integer, UUID) or final fields in hashCode." },
  { q: "TreeMap vs HashMap — when do you choose TreeMap?", opts: ["TreeMap when you need sorted key order, range queries, or floor/ceiling operations — O(log n) vs HashMap O(1)", "HashMap doesn't support get()", "TreeMap uses less memory", "TreeMap is always faster"], ans: 0, exp: "TreeMap (Red-Black tree): keys always sorted, supports floorKey(), ceilingKey(), subMap(), headMap(), tailMap() — perfect for range queries. O(log n) all ops. HashMap O(1) but unordered. Use TreeMap when order matters." },
  { q: "How to count word frequencies in Java using Map?", opts: ["Use a List and sort", "Use indexOf on each word", "map.merge(word, 1, Integer::sum) or getOrDefault pattern", "Sort array first"], ans: 2, exp: "Clean idiom: map.merge(word, 1, Integer::sum) — if key absent, puts 1; if present, applies the merging function (adds 1). Alternative: map.put(word, map.getOrDefault(word, 0) + 1). merge() is cleaner and more readable." }
])}
...
${quizHTML('ds-trees', [
  { q: "Inorder traversal of a BST gives?", opts: ["Elements in sorted ascending order", "Random order", "Level-by-level order", "Elements in reverse sorted order"], ans: 0, exp: "BST property: left < root < right. Inorder (left → root → right) visits nodes in ascending sorted order. This is why 'validate BST' and 'kth smallest in BST' both use inorder traversal." },
  { q: "BFS vs DFS — when to use each for trees?", opts: ["BFS: level-order, shortest path. DFS: path problems, backtracking, all paths", "They give identical results always", "Always use DFS", "Always use BFS"], ans: 0, exp: "BFS (queue): explores level by level — best for 'minimum depth', 'level order traversal', 'shortest path'. DFS (recursion/stack): goes deep first — best for 'all paths', 'path sum', 'height', 'validate BST'. Choose based on what the problem needs." },
  { q: "Height of a balanced binary tree with n nodes?", opts: ["O(n²)", "O(log n)", "O(1)", "O(n)"], ans: 1, exp: "A balanced binary tree has O(log n) height. Each level doubles node count: level 0=1, level 1=2, level 2=4 → height = log₂(n). Unbalanced (degenerate/linked-list shape) → O(n) height. This is why AVL/Red-Black trees are important." },
  { q: "What is a Trie best used for?", opts: ["Prefix-based search (autocomplete, spell check) — O(m) lookup where m = word length", "Balanced sorting", "Graph traversal", "Sorting integers"], ans: 0, exp: "Trie (prefix tree): each node represents a character. Common prefixes share nodes. insert/search/startsWith all O(m) where m=word length — independent of dictionary size. Used in autocomplete, spell checkers, IP routing (prefix matching)." },
  { q: "Lowest Common Ancestor (LCA) of two nodes — approach?", opts: ["Recursive: if root is null or equals either node, return root; recurse both sides; if both non-null, root is LCA", "Use inorder traversal", "BFS from both nodes", "Sort nodes by value"], ans: 0, exp: "Recursive LCA: search left and right subtrees. If both return non-null → current node is LCA (nodes are on different sides). If only one returns non-null → both nodes in same subtree (return that result). O(n) time." },
  { q: "Java TreeMap vs TreeSet — difference?", opts: ["TreeSet is faster", "No difference", "TreeMap: sorted key-value pairs. TreeSet: sorted unique keys only (backed by TreeMap with dummy values)", "TreeMap doesn't sort"], ans: 2, exp: "Both backed by Red-Black tree, O(log n) ops. TreeMap: key→value store with sorted keys, supports floorEntry(), ceilingEntry(), subMap(). TreeSet: unique sorted elements, supports floor(), ceiling(), subset(). TreeSet is TreeMap<K, Boolean>." },
  { q: "Max depth vs diameter of a binary tree?", opts: ["They are the same", "Max depth: longest root-to-leaf path. Diameter: longest path between any two nodes (may not pass through root)", "Diameter counts edges not nodes", "Diameter is always twice max depth"], ans: 1, exp: "Max depth: longest root-to-leaf chain. Diameter: longest path between ANY two nodes — could go through left subtree, through root, into right subtree. Diameter = max(left_height + right_height) for each node. Both solved with DFS." },
  { q: "Heap vs BST — which to prefer for 'find minimum'?", opts: ["Min-heap: O(1) peek minimum. BST: O(log n) to find leftmost. Heap wins for top-K and priority queue", "Depends on language", "BST always", "They are equivalent"], ans: 0, exp: "Min-heap peek(): O(1) — root is always minimum. Insertion/removal: O(log n). BST minimum: O(log n) traverse left. For priority queue, repeated find-min: heap is better. For range queries/sorted iteration: BST is better." },
  { q: "Serialize and deserialize a binary tree — which traversal?", opts: ["Inorder alone (ambiguous)", "Postorder only", "Level-order only", "Preorder (root-left-right) with null markers — uniquely identifies tree structure"], ans: 3, exp: "Inorder alone is ambiguous (multiple trees can have same inorder). Preorder with null markers ('1,2,null,null,3,null,null') uniquely encodes structure. During deserialization, split by commas and recursively build tree. Level-order (BFS) also works." }
])}
...
${quizHTML('ds-graph', [
  { q: "BFS vs DFS for finding shortest path in unweighted graph?", opts: ["Dijkstra only", "Both give same shortest path", "DFS is better", "BFS guarantees shortest path in unweighted graphs — explores level by level"], ans: 3, exp: "BFS explores nodes level by level (distance 1, then 2, then 3...). First time you reach destination = shortest path. DFS may find a path but not the shortest. For weighted graphs, use Dijkstra (non-negative weights) or Bellman-Ford." },
  { q: "How to detect a cycle in a directed graph?", opts: ["DFS with 3-color marking: white (unvisited), gray (in progress), black (done) — back edge to gray = cycle", "Union-Find", "Topological sort only", "BFS from every node"], ans: 0, exp: "During DFS: gray = currently in the call stack. If you encounter a gray node, you found a back edge = cycle. White = not visited, black = fully processed (no cycle through this path). Time: O(V+E)." },
  { q: "Topological sort — what graph property is required?", opts: ["Graph must be undirected", "Graph must be connected", "Graph must be a DAG (Directed Acyclic Graph)", "Graph must have equal in/out degrees"], ans: 2, exp: "Topological sort only works on DAGs. Ordering: for every edge u→v, u appears before v. Used for: build dependency order, course prerequisites, task scheduling. Two algorithms: DFS (reverse finish order) or Kahn's (BFS with in-degree tracking)." },
  { q: "Number of islands problem — best approach?", opts: ["Dynamic programming", "Sort the grid", "DFS/BFS flood fill: for each unvisited '1', run DFS marking all connected '1's as visited, increment count", "Union-Find only"], ans: 2, exp: "Flood fill DFS: iterate grid, when '1' found, do DFS to mark entire island as visited ('0' or separate visited array), increment count. O(m×n) time. Union-Find is an alternative — connect adjacent '1's, count components." },
  { q: "Dijkstra's algorithm fails when?", opts: ["Graph has negative edge weights", "Graph is undirected", "Graph is disconnected", "Graph has more than 1000 nodes"], ans: 0, exp: "Dijkstra's greedy assumption: once a node is finalized, its distance is correct. With negative edges, a later path could improve an already-finalized distance. Use Bellman-Ford for negative weights (O(VE)) or Floyd-Warshall for all-pairs." },
  { q: "Adjacency list vs adjacency matrix — when to use matrix?", opts: ["Dense graphs (E ≈ V²) — O(1) edge lookup vs O(degree) for list. For sparse graphs, list is better (memory: V² vs V+E)", "List is always better", "Matrix is always faster", "Always use matrix"], ans: 0, exp: "Adjacency matrix: O(V²) space, O(1) edge check. Good for dense graphs. Adjacency list: O(V+E) space, O(degree) edge check. Good for sparse graphs (E << V²). Most real graphs (social networks, web) are sparse — use adjacency list." },
  { q: "Union-Find (Disjoint Set) is optimised with?", opts: ["BFS traversal", "Path compression + union by rank — nearly O(1) amortized (O(α(n)) inverse Ackermann)", "Linked list backing", "Sorting before union"], ans: 1, exp: "Path compression: when finding root, make all nodes on path point directly to root — flattens tree. Union by rank: always attach smaller tree under larger — keeps tree shallow. Combined: nearly O(1) per operation (O(α(n)) which is ≤5 for all practical n)." },
  { q: "What algorithm finds the Minimum Spanning Tree?", opts: ["Kruskal's (sort edges by weight, add if no cycle — Union-Find) or Prim's (greedy grow from one node)", "Topological sort", "BFS from all nodes", "Dijkstra"], ans: 0, exp: "Kruskal's: sort all edges by weight, use Union-Find to add edge if it connects two different components. O(E log E). Prim's: start at any node, always add cheapest edge connecting visited to unvisited. Both give MST, similar complexity." },
  { q: "Graph coloring with 2 colors — what graph property does this test?", opts: ["Whether graph is bipartite — possible iff no odd-length cycles", "Whether graph has Euler circuit", "Whether graph is planar", "Whether graph is connected"], ans: 0, exp: "Bipartite check: try to 2-color graph with BFS/DFS. If you can color all adjacent nodes different colors, it's bipartite. Fails iff there's an odd-length cycle. Used in: matching problems, scheduling, Netflix recommendation (users and movies as two node sets)." }
])}
...
${quizHTML('ds-heap', [
  { q: "Min-heap vs max-heap — which is used for 'find K largest elements'?", opts: ["Min-heap of size K — smallest of the K largest is at root, compare each new element against root", "Binary search tree", "Max-heap of size N", "Sorted array"], ans: 0, exp: "Keep a min-heap of size K. For each element: if larger than root, remove root and insert element. At end, heap contains K largest elements. Root = Kth largest. O(N log K) time, O(K) space." },
  { q: "Time complexity of heapify (building heap from array)?", opts: ["O(N²)", "O(log N)", "O(N) — bottom-up sift-down is linear, not N × O(log N)", "O(N log N)"], ans: 2, exp: "Common mistake: thinking heapify = N inserts = O(N log N). Bottom-up heapify: start from last non-leaf, sift down. Most nodes are near bottom (short sift). Mathematical proof: sum converges to O(N)." }
])}
...
<div class="card-grid">
  ...
  <div class="card" onclick="showPage('algo-fastslow')">
    <div class="card-icon">🐢🐇</div>
  ...
  <div class="card" onclick="showPage('algo-prefix')">
    <div class="card-icon">📊</div>
  ...
  <div class="card" onclick="showPage('algo-matrix')">
    <div class="card-icon">🔲</div>
...
<div class="callout callout-blue" style="margin-top:24px">
  ...
  <strong>"cycle" / "middle of list"</strong> → Fast &amp; Slow.
...
${quizHTML('algo-patterns', [
  { q: "Which pattern to use when 'find all subsets/permutations'?", opts: ["Two pointers", "Sliding window", "Binary search", "Backtracking — explore all choices recursively, undo each choice before trying next"], ans: 3, exp: "Backtracking = systematic brute force with pruning. Build solution incrementally, abandon paths that cannot lead to valid solution. Subsets, permutations, N-Queens, Sudoku all use this pattern." },
  { q: "Two Pointers vs Fast & Slow Pointers — key difference?", opts: ["They are identical", "Classic Two Pointers moves opposite directions on sorted arrays (pair sums, palindromes). Fast & Slow moves same direction at different speeds, almost always on linked lists (cycle, middle).", "Two pointers is always slower", "Fast & Slow only works on sorted arrays"], ans: 1, exp: "Both use 2 pointers, but problem signatures differ. Classic Two Pointers: sorted array + find pair/palindrome → pointers at opposite ends moving toward center. Fast & Slow: linked list + cycle/middle → same-start pointers moving at 1x and 2x. Recognizing the right variant is how you pick the right pattern quickly." },
  { q: "Sliding window vs two pointers — when to reach for sliding window?", opts: ["Never", "When the problem asks for a contiguous subarray/substring that satisfies a condition — expand right, shrink left when constraint violated", "Only for sorted arrays", "Only for linked lists"], ans: 1, exp: "Sliding window is a specialization: you maintain a contiguous window that satisfies a constraint. Right pointer expands the window, left pointer shrinks it when the constraint is violated. Keywords: longest/shortest substring, maximum sum of size-k subarray." }
])}
`;
...
<div class="callout callout-amber">
  <strong>vs Fast &amp; Slow Pointers:</strong> Both patterns use 2 pointers, but the problem signatures are different. <strong>Classic Two Pointers</strong> (this page) starts at opposite ends of a sorted array and moves toward the center — used for pair sums, palindromes, container problems. <strong>Fast &amp; Slow Pointers</strong> starts both at the same position and moves at different speeds (1× and 2×) — used for cycle detection and finding the middle of a linked list. See <a onclick="showPage('algo-fastslow')" style="color:var(--accent);cursor:pointer;text-decoration:underline">Fast &amp; Slow Pointers</a> for that variant.
</div>

<div class="callout callout-blue">
  <strong>The Trick:</strong> Make the comparison at <code>arr[left]</code> and <code>arr[right]</code> tell you which pointer to move. If the pair sums to <em>less than target</em>, need a bigger sum → <code>left++</code> (next larger). If <em>more</em>, need smaller → <code>right--</code>. On a sorted array this is guaranteed to find the answer in O(n) because each pointer moves only one direction and meets in the middle.
</div>
...
${quizHTML('algo-twopointer', [
  { q: "Two pointer pattern requires what array property?", opts: ["Array must have even length", "Array must have unique elements", "Array must be unsorted", "Array must be sorted (convergence from both ends is only valid when sorted)"], ans: 3, exp: "Two pointers converge because sorted order guarantees: moving left pointer right increases sum, moving right pointer left decreases sum. Without sorted order you cannot make this greedy decision. Always sort first if needed." },
  { q: "Container With Most Water — why move the SHORTER pointer?", opts: ["Random choice", "Both pointers should move", "Moving the taller pointer can only decrease area since width shrinks and height stays limited by the shorter side", "The shorter pointer is always wrong"], ans: 2, exp: "Area = min(left, right) x width. If you move the taller pointer: min() is still the shorter side, width decreases → area decreases. Moving shorter pointer: width decreases but you might find a taller wall → possible improvement. So always move shorter." },
  { q: "3Sum — how to avoid duplicate triplets?", opts: ["Sort array, skip duplicate values at each pointer position after finding a valid triplet", "Use a HashSet for all triplets", "Use HashMap to count frequencies", "Sort by absolute value"], ans: 0, exp: "After sorting: outer loop skips if nums[i] == nums[i-1]. When triplet found, skip duplicates for both left and right (while nums[left]==nums[left+1]: left++). O(n^2) total with deduplication handled inline." },
  { q: "Slow/fast pointer: after finding cycle, how to find cycle START?", opts: ["Count nodes in cycle", "Reset slow to head, advance both one step at a time — meeting point is cycle start", "Use a HashSet", "Keep fast at meeting point"], ans: 1, exp: "After detection, reset slow to head. Advance both pointers 1 step at a time. They meet exactly at the cycle start. Mathematical proof: distance head-to-start equals distance meeting-point-to-start within the cycle." },
  { q: "Move all zeros to end — two pointer approach?", opts: ["Non-zero pointer at 0, scan pointer advances; when non-zero found swap with non-zero pointer position", "Count zeros first", "Sort the array", "Use extra array"], ans: 0, exp: "Left pointer tracks insert position for non-zeros. Right pointer scans. When right is non-zero: swap with left, advance left. O(n) time O(1) space. All non-zeros move left, zeros fill the rest." },
  ...
  { q: "Remove duplicates from sorted array in-place?", opts: ["Slow pointer tracks last unique; fast pointer finds next unique — copy to slow+1", "Count occurrences first", "Use a set", "Sort first"], ans: 0, exp: "Slow=0 (last written), fast scans. When nums[fast] != nums[slow]: slow++, copy nums[fast] to nums[slow]. Return slow+1 as count of unique elements. In-place O(n) time O(1) space." },
  { q: "Merge two sorted arrays in-place — which direction?", opts: ["Sort after copying", "Right to left — start from the end to avoid overwriting elements not yet processed", "Doesn't matter", "Left to right"], ans: 1, exp: "Merging left-to-right into array A overwrites elements not yet compared. Merge right-to-left: start from end of both, place larger at end of A. No overwrites possible. O(m+n) time O(1) space." },
  { q: "Valid palindrome — two pointer handling of non-alphanumeric chars?", opts: ["Convert to lowercase first", "Always return false if non-alnum present", "Two pointers skip non-alphanumeric inline — O(1) extra space", "Remove all non-alnum first O(n) space"], ans: 2, exp: "While left < right: if not alphanumeric, skip that pointer. Else compare toLowerCase. Skip-inline approach: O(n) time O(1) space — no extra string creation. This is the canonical O(1) space palindrome check." }
])}
`;
...
<div class="callout callout-blue">
  <strong>The Trick:</strong> Instead of recomputing the sum/count for every window from scratch (O(n·k)), <strong>slide incrementally</strong>: when the window moves one step right, ADD the new element and REMOVE the leftmost one — each update is O(1), giving O(n) overall. For variable-size windows, expand right until the constraint breaks, then shrink left until it holds again. Every element enters and leaves the window at most once.
</div>
...
${quizHTML('algo-sliding', [
  { q: "When to use sliding window?", opts: ["When finding optimal contiguous subarray/substring — max sum of size K, longest substring without repeating, etc.", "Any array problem", "When elements need sorting", "Only for sorted arrays"], ans: 0, exp: "Sliding window keywords: 'contiguous', 'subarray', 'substring', 'window of size K', 'longest/shortest with condition'. Fixed-size window: move both pointers. Variable-size: expand right, shrink left when constraint breaks." },
  { q: "Time complexity of sliding window?", opts: ["O(N) — each element is added and removed at most once", "O(2^N)", "O(N log N)", "O(N²)"], ans: 0, exp: "Despite having a while loop inside the for loop, each element enters the window once (right pointer) and leaves once (left pointer). Total operations = 2N = O(N). This is the key insight interviewers look for." }
])}
...
<div class="callout callout-blue">
  <strong>The Trick:</strong> Binary search doesn't require a sorted array — it requires a <strong>monotonic condition</strong>. Ask: "Is there a value X where <em>everything ≤ X is feasible and everything &gt; X is not</em>?" If yes, binary search the answer space. This unlocks problems like "minimum capacity to ship in D days", "split array into K subarrays with min largest sum", "smallest divisor". The search space is values, not array indices.
</div>

<div class="memory-tip" style="margin-top:10px">
  <div class="mt-label">🧠 Memorize</div>
  <div class="mt-content"><strong>Two templates, two loop conditions:</strong> exact-match uses <code>lo &lt;= hi</code> and returns <code>mid</code> when found. Boundary search (first-true, minimum-that-works) uses <code>lo &lt; hi</code> and returns <code>lo</code> at the end. Pick one, commit to it, don't mix.</div>
</div>
...
${quizHTML('algo-binsearch', [
  { q: "Binary search on answer space — what does that mean?", opts: ["The answer itself is monotonic — binary search on possible answer values, check if each candidate is feasible", "Searching a sorted array", "Searching a 2D matrix", "Using two binary searches"], ans: 0, exp: "Example: 'minimum capacity to ship packages in D days'. Answer range: max(weights) to sum(weights). Binary search on capacity value, for each candidate check if it's feasible (can ship in ≤D days). Works when: answer is monotonic (if X works, X+1 also works)." },
  { q: "lo < hi vs lo <= hi — when to use which?", opts: ["No difference", "lo < hi when converging to a single point (search space shrinks to 1). lo <= hi when searching for exact match.", "lo < hi is faster", "Always use lo <= hi"], ans: 1, exp: "lo <= hi: classic search, return mid when found. lo < hi: find insertion point or boundary — loop ends when lo == hi == answer. Using wrong one causes infinite loops or off-by-one errors. Practice both patterns." }
])}
...
<div class="page-header">
  ...
  <div class="page-subtitle">Two ways to traverse graphs and trees. BFS explores layer by layer (queue). DFS dives deep before backtracking (stack / recursion).</div>
</div>

<div class="memory-tip">
  <div class="mt-label">🎯 Trigger Keywords</div>
  <div class="mt-content"><strong>BFS:</strong> "shortest path (unweighted)", "level order", "minimum steps to reach", "spreading / rotting". <strong>DFS:</strong> "all paths", "connected components", "cycle detection", "topological sort", "combinatorial search". If shortest / level → BFS. If exhaustive / deep → DFS.</div>
</div>

<div class="callout callout-blue">
  <strong>The Trick (how to memorize):</strong> <strong>BFS = ripple in a pond</strong> — the wavefront expands uniformly outward, so the first time you reach any cell is the shortest distance. <strong>DFS = spelunking</strong> — go down one tunnel as far as possible, backtrack only when stuck. BFS uses a <strong>queue (FIFO)</strong>; DFS uses a <strong>stack or recursion (LIFO)</strong>. The queue-vs-stack swap is literally the only code difference.
</div>
...
<div class="two-col">
  <div class="info-box">
    ...
    <ul class="content-list" style="margin-top:6px">
      <li>Shortest path in unweighted graph/grid</li>
      <li>Level-order tree traversal</li>
      <li>Spreading problems (rotting oranges, virus)</li>
  ...
  <div class="info-box">
    ...
    <ul class="content-list" style="margin-top:6px">
      <li>Connected components / count islands</li>
      <li>Cycle detection (directed &amp; undirected)</li>
      <li>Topological sort (post-order DFS)</li>
      <li>Path existence, maze solving, backtracking</li>
    </ul>
  </div>
</div>

<div class="section-title">BFS Template — Queue</div>
<div class="code-block"><pre><span class="cm">// Generic BFS — returns shortest distance from start to target</span>
<span class="kw">int</span> bfs(Node start, Node target) {
    Queue&lt;Node&gt; queue = <span class="kw">new</span> ArrayDeque&lt;&gt;();
    Set&lt;Node&gt; visited = <span class="kw">new</span> HashSet&lt;&gt;();
    queue.offer(start);
    visited.add(start);
    <span class="kw">int</span> steps = <span class="num">0</span>;

    <span class="kw">while</span> (!queue.isEmpty()) {
        <span class="kw">int</span> size = queue.size();         <span class="cm">// process one level at a time</span>
        <span class="kw">for</span> (<span class="kw">int</span> i = <span class="num">0</span>; i &lt; size; i++) {
            Node cur = queue.poll();
            <span class="kw">if</span> (cur.equals(target)) <span class="kw">return</span> steps;
            <span class="kw">for</span> (Node next : cur.neighbors()) {
                <span class="kw">if</span> (!visited.contains(next)) {
                    visited.add(next);
                    queue.offer(next);
                }
            }
        }
        steps++;                         <span class="cm">// finished one level</span>
    }
    <span class="kw">return</span> -<span class="num">1</span>;                       <span class="cm">// target unreachable</span>
}</pre></div>
<div class="callout callout-amber">
  <strong>Why track level size?</strong> The <code>for i &lt; size</code> loop processes exactly one level before incrementing <code>steps</code>. That's how BFS returns correct distances. Skipping this gives you BFS that visits everything but can't tell you distance.
</div>

<div class="section-title">DFS Template — Recursion</div>
<div class="code-block"><pre><span class="cm">// DFS on a graph — find all connected nodes from start</span>
<span class="kw">void</span> dfs(Node cur, Set&lt;Node&gt; visited) {
    <span class="kw">if</span> (visited.contains(cur)) <span class="kw">return</span>;
    visited.add(cur);
    <span class="cm">// process cur (pre-order)</span>
    <span class="kw">for</span> (Node next : cur.neighbors()) {
        dfs(next, visited);
    }
    <span class="cm">// process cur (post-order — used in topological sort)</span>
}

<span class="cm">// DFS iterative — explicit stack (use when recursion depth might blow the stack)</span>
<span class="kw">void</span> dfsIterative(Node start) {
    Deque&lt;Node&gt; stack = <span class="kw">new</span> ArrayDeque&lt;&gt;();
    Set&lt;Node&gt; visited = <span class="kw">new</span> HashSet&lt;&gt;();
    stack.push(start);
    <span class="kw">while</span> (!stack.isEmpty()) {
        Node cur = stack.pop();
        <span class="kw">if</span> (!visited.add(cur)) <span class="kw">continue</span>;
        <span class="kw">for</span> (Node next : cur.neighbors()) {
            <span class="kw">if</span> (!visited.contains(next)) stack.push(next);
        }
    }
}</pre></div>

<div class="section-title">Tree BFS — Level Order (LeetCode 102)</div>
<div class="code-block"><pre><span class="kw">public</span> List&lt;List&lt;Integer&gt;&gt; levelOrder(TreeNode root) {
    ...
    Queue&lt;TreeNode&gt; queue = <span class="kw">new</span> ArrayDeque&lt;&gt;();
    queue.offer(root);
    <span class="kw">while</span> (!queue.isEmpty()) {
...
<div class="callout callout-blue">
  <strong>Trees vs graphs:</strong> On trees you don't need a visited set — there are no cycles. On graphs you must track visited to avoid infinite loops. That's the only structural difference in the templates.
</div>

<div class="section-title">Cycle Detection in Directed Graph (DFS + 3 colors)</div>
<div class="code-block"><pre><span class="cm">// WHITE = unvisited, GRAY = on current DFS path, BLACK = fully processed</span>
<span class="kw">int</span>[] color;  <span class="cm">// 0 = WHITE, 1 = GRAY, 2 = BLACK</span>

<span class="kw">boolean</span> hasCycle(List&lt;List&lt;Integer&gt;&gt; graph) {
    color = <span class="kw">new int</span>[graph.size()];
    <span class="kw">for</span> (<span class="kw">int</span> i = <span class="num">0</span>; i &lt; graph.size(); i++) {
        <span class="kw">if</span> (color[i] == <span class="num">0</span> &amp;&amp; dfs(i, graph)) <span class="kw">return true</span>;
    }
    <span class="kw">return false</span>;
}

<span class="kw">boolean</span> dfs(<span class="kw">int</span> u, List&lt;List&lt;Integer&gt;&gt; graph) {
    color[u] = <span class="num">1</span>;                       <span class="cm">// GRAY — on current path</span>
    <span class="kw">for</span> (<span class="kw">int</span> v : graph.get(u)) {
        <span class="kw">if</span> (color[v] == <span class="num">1</span>) <span class="kw">return true</span>;   <span class="cm">// back edge → cycle</span>
        <span class="kw">if</span> (color[v] == <span class="num">0</span> &amp;&amp; dfs(v, graph)) <span class="kw">return true</span>;
    }
...
<div class="callout callout-green">
  <strong>Classic problems:</strong>
  <strong>BFS</strong> — Binary Tree Level Order (102), Shortest Path in Binary Matrix (1091), Rotting Oranges (994), Word Ladder (127), Open the Lock (752).
  <strong>DFS</strong> — Number of Islands (200), Clone Graph (133), Course Schedule (207 — cycle detection), Pacific Atlantic Water Flow (417), Path Sum (112).
</div>

${revisionControls('algo-bfsdfs')}

${quizHTML('algo-bfsdfs', [
  { q: "BFS vs DFS — when to prefer BFS?", opts: ["When finding shortest path in an unweighted graph — BFS guarantees shortest path, DFS does not", "Always use BFS", "DFS cannot handle cycles", "BFS uses less memory"], ans: 0, exp: "BFS explores level by level → first time you reach a node is the shortest path (in unweighted graphs). DFS may find a longer path first. BFS uses O(V) memory (queue), DFS uses O(depth) stack. Use DFS for: exhaustive search, backtracking, detecting cycles, topological sort." },
  { q: "How to detect a cycle in a directed graph?", opts: ["DFS with 3 states: unvisited, in-progress (on current path), visited. Back edge to in-progress node = cycle", "Topological sort only", "Check if edge count > vertex count", "BFS only"], ans: 0, exp: "Color each node: WHITE (unvisited), GRAY (in current DFS path), BLACK (fully processed). If DFS encounters a GRAY node, there's a cycle on the current path. More precise than plain 'visited' — it distinguishes back edges from cross edges." },
  { q: "Why does BFS process one level at a time using the queue.size() trick?", opts: ["To confuse readers", "So you can increment steps/depth exactly once per level — essential for returning the correct shortest distance or building level-order output", "Queues require it", "It does not matter"], ans: 1, exp: "Snapshot queue.size() at the top of the outer while loop, then poll that many nodes and enqueue their neighbors. After the inner for-loop, exactly one level has been processed, so you can increment steps (for shortest path) or start a new sub-list (for level order). Without this pattern, nodes from multiple levels mix in the queue and you lose distance info." }
])}
...
<div class="callout callout-blue">
  <strong>The Trick (how to memorize):</strong> Three steps repeated at every level — <strong>Choose → Explore → Un-choose</strong>. Add element to current partial solution, recurse, then REMOVE it before trying the next option. That "un-choose" step is the backtrack — it's what lets you explore the full tree of choices with a single shared <code>curr</code> list. Pruning (early <code>return</code> or <code>continue</code> on invalid states) is what turns O(n!) worst case into something fast enough to pass.
</div>
...
${quizHTML('algo-backtrack', [
  { q: "Backtracking template — what are the 3 key steps?", opts: ["Hash, compare, merge", "Choose (add to path) → Explore (recurse) → Unchoose (remove from path, try next option)", "Sort, search, return", "Divide, conquer, combine"], ans: 1, exp: "Every backtracking problem: 1) Choose: add element to current solution. 2) Explore: recurse with remaining choices. 3) Unchoose: remove element, restore state, try next option. The 'unchoose' step is what distinguishes backtracking from plain recursion." },
  { q: "How to avoid duplicate subsets in backtracking?", opts: ["Use a HashSet", "Sort the array first, then skip consecutive duplicates at each recursion level: if (i > start && nums[i] == nums[i-1]) skip", "Remove duplicates from input", "Use a different algorithm"], ans: 1, exp: "Sort input array. At each level, skip nums[i] if it equals nums[i-1] AND i > start (meaning we already used this value at this position). This prunes duplicate branches without needing a HashSet. Works for subsets, combinations, permutations." }
])}
...
<div class="memory-tip">
  <div class="mt-label">🎯 Trigger Keywords</div>
  <div class="mt-content">"top K largest / smallest", "K most frequent", "K closest points", "Kth element". The answer only needs K items, not a full sort — that's when heap wins over sorting.</div>
</div>
...
${quizHTML('algo-topk', [
  { q: "Top-K elements — heap vs quickselect?", opts: ["Always use sorting", "They give different results", "Heap: O(N log K) guaranteed. Quickselect: O(N) average but O(N²) worst case. Heap for streaming data, quickselect for in-memory.", "Quickselect is always better"], ans: 2, exp: "Min-heap of size K: process each element O(log K), total O(N log K), works with streaming data. Quickselect (modified quicksort): partition around pivot, recurse only on the side containing Kth element. O(N) average, but worst case O(N²) without random pivot." }
])}
...
// ═══════════════════════════════════════════════
pages['algo-fastslow'] = () => `
<div class="breadcrumb">Algorithms → <span>Fast &amp; Slow Pointers</span></div>
<div class="page-header">
  <div class="page-title">Fast &amp; Slow Pointers (Tortoise &amp; Hare)</div>
  <div class="page-subtitle">Two pointers starting at the same position but moving at different speeds — the classic technique for cycle detection in linked lists.</div>
</div>

<div class="memory-tip">
  <div class="mt-label">🎯 Trigger Keywords</div>
  <div class="mt-content">"linked list cycle", "find middle node", "nth from end", "happy number", "palindrome linked list". If the problem is on a <em>linked list</em> and needs a second traversal position — think fast &amp; slow.</div>
</div>

<div class="callout callout-amber">
  <strong>vs Two Pointers:</strong> Classic Two Pointers (on sorted arrays) moves pointers toward each other from opposite ends. Fast &amp; Slow moves pointers in the <em>same direction</em> at <em>different speeds</em>, almost always on linked lists where you can't index into the middle. Same idea (2 pointers) but very different problem signatures.
</div>

<div class="section-title">The Trick</div>
<div class="code-block"><pre><span class="cm">// slow = 1 step, fast = 2 steps</span>
<span class="cm">// • If there's a cycle → fast laps slow → they meet</span>
<span class="cm">// • If no cycle → fast hits null first</span>
...
<span class="kw">while</span> (fast != <span class="kw">null</span> &amp;&amp; fast.next != <span class="kw">null</span>) {
    slow = slow.next;
    fast = fast.next.next;
    <span class="kw">if</span> (slow == fast) <span class="cm">/* cycle detected */</span> ;
}
<span class="cm">// after loop: slow is at middle, fast is at end (or null)</span></pre></div>

<div class="section-title">Cycle Detection (LeetCode 141)</div>
<div class="code-block"><pre><span class="kw">public boolean</span> hasCycle(ListNode head) {
    ListNode slow = head, fast = head;
    <span class="kw">while</span> (fast != <span class="kw">null</span> &amp;&amp; fast.next != <span class="kw">null</span>) {
        slow = slow.next;
        fast = fast.next.next;
...
<div class="callout callout-blue">
  <strong>Why it works:</strong> Inside a cycle, the gap between slow and fast closes by 1 each step (fast advances 2, slow advances 1 → net +1). So fast catches up in at most L steps where L = cycle length. No cycle → fast reaches null first.
</div>

<div class="section-title">Find Start of Cycle (LeetCode 142)</div>
<div class="code-block"><pre><span class="cm">// Floyd's algorithm: after slow &amp; fast meet inside cycle,</span>
<span class="cm">// reset one pointer to head and advance both at same speed.</span>
<span class="cm">// They meet again at the cycle start.</span>
<span class="kw">public</span> ListNode detectCycle(ListNode head) {
    ListNode slow = head, fast = head;
    <span class="kw">while</span> (fast != <span class="kw">null</span> &amp;&amp; fast.next != <span class="kw">null</span>) {
        slow = slow.next;
        fast = fast.next.next;
        <span class="kw">if</span> (slow == fast) {                 <span class="cm">// Phase 1: find meeting point</span>
            ListNode p = head;
            <span class="kw">while</span> (p != slow) {              <span class="cm">// Phase 2: walk both at 1x</span>
                p = p.next;
                slow = slow.next;
            }
            <span class="kw">return</span> p;                         <span class="cm">// start of cycle</span>
        }
    }
    <span class="kw">return null</span>;
}</pre></div>

<div class="section-title">Middle of Linked List (LeetCode 876)</div>
<div class="code-block"><pre><span class="kw">public</span> ListNode middleNode(ListNode head) {
    ListNode slow = head, fast = head;
    <span class="kw">while</span> (fast != <span class="kw">null</span> &amp;&amp; fast.next != <span class="kw">null</span>) {
        slow = slow.next;
        fast = fast.next.next;
    }
    <span class="kw">return</span> slow;  <span class="cm">// for even length, this is the 2nd middle (n/2 + 1)</span>
}</pre></div>

<div class="callout callout-green">
  <strong>Classic problems:</strong> Linked List Cycle (141), Cycle II — find start (142), Middle of LL (876), Happy Number (202), Palindrome LL (234), Remove Nth from End (19 — uses a gap variant), Reorder List (143).
</div>

${revisionControls('algo-fastslow')}

${quizHTML('algo-fastslow', [
  { q: "In Floyd's cycle detection, why does resetting one pointer to head and walking both at 1x find the cycle start?", opts: ["Random chance", "It does not — you need extra space", "Math: if distance from head to cycle start is D and meeting point is K steps into the cycle, then D = L - K (mod L). Walking both at 1x from head and meeting point, they meet at the cycle start.", "Only works for specific cycle lengths"], ans: 2, exp: "When slow and fast meet, slow has traveled D + K steps (D = head to cycle start, K = position in cycle). Fast has traveled 2(D + K) but is at the same node, so cycle length L divides D + K. From meeting point, walking L - K more steps reaches cycle start. From head, walking D steps reaches cycle start. And D = L - K means both pointers meet there." },
  { q: "How does Fast & Slow differ from classic Two Pointers?", opts: ["They are identical", "Classic Two Pointers moves opposite directions on sorted arrays (pair sums, palindromes). Fast & Slow moves same direction at different speeds, almost always on linked lists (cycle, middle).", "Fast & Slow is always slower", "Classic Two Pointers only works on linked lists"], ans: 1, exp: "Both use 2 pointers, but problem signatures differ. Classic Two Pointers: sorted array + find pair/palindrome → pointers at opposite ends moving toward center. Fast & Slow: linked list + cycle/middle → same-start pointers moving at 1x and 2x. Recognizing the right variant is how you pick the right pattern quickly." }
])}

`;

pages['algo-prefix'] = () => `
<div class="breadcrumb">Algorithms → <span>Prefix Sum</span></div>
<div class="page-header">
  <div class="page-title">Prefix Sum Pattern</div>
  <div class="page-subtitle">Precompute cumulative sums to answer unlimited range-sum queries in O(1) each, at the cost of O(n) preprocessing.</div>
</div>

<div class="memory-tip">
  <div class="mt-label">🎯 Trigger Keywords</div>
  <div class="mt-content">"range sum", "subarray sum equals K", "running sum", "equilibrium index", "number of subarrays with sum divisible by K", "product of array except self" (same idea with products). Any time you see <em>many</em> queries over ranges of a static array.</div>
</div>

<div class="section-title">The Trick</div>
<div class="code-block"><pre><span class="cm">// prefix[i] = sum of arr[0..i-1]  (index-shifted for easy range math)</span>
...
<span class="cm">// Range sum arr[l..r] inclusive, in O(1)</span>
<span class="kw">int</span> rangeSum = prefix[r + <span class="num">1</span>] - prefix[l];</pre></div>
<div class="callout callout-blue">
  <strong>Why the +1 offset?</strong> Using <code>prefix[0] = 0</code> as an empty-prefix sentinel avoids special-casing <code>l == 0</code>. The formula <code>prefix[r+1] - prefix[l]</code> handles every case uniformly.
</div>

<div class="section-title">Subarray Sum Equals K (LeetCode 560)</div>
<div class="callout callout-amber">
  <strong>Key insight:</strong> A subarray summing to K ending at index <code>j</code> means some prefix sum at index <code>i &lt; j</code> equals <code>prefix[j] - K</code>. Use a HashMap to count how many times each prefix sum has been seen.
</div>
<div class="code-block"><pre><span class="kw">public int</span> subarraySum(<span class="kw">int</span>[] nums, <span class="kw">int</span> k) {
    ...
    <span class="kw">for</span> (<span class="kw">int</span> n : nums) {
        sum += n;
        count += seen.getOrDefault(sum - k, <span class="num">0</span>);
        seen.merge(sum, <span class="num">1</span>, Integer::sum);
    }
    <span class="kw">return</span> count;
}</pre></div>

<div class="section-title">2D Prefix Sum (LeetCode 304)</div>
<div class="code-block"><pre><span class="cm">// sum[i+1][j+1] = sum of matrix[0..i][0..j]</span>
<span class="kw">for</span> (<span class="kw">int</span> i = <span class="num">0</span>; i &lt; m; i++)
  <span class="kw">for</span> (<span class="kw">int</span> j = <span class="num">0</span>; j &lt; n; j++)
    sum[i+<span class="num">1</span>][j+<span class="num">1</span>] = matrix[i][j]
                   + sum[i][j+<span class="num">1</span>]      <span class="cm">// row above</span>
                   + sum[i+<span class="num">1</span>][j]      <span class="cm">// col left</span>
                   - sum[i][j];         <span class="cm">// subtract double-counted corner</span>

<span class="cm">// Query submatrix (r1,c1) to (r2,c2) in O(1)</span>
<span class="kw">int</span> region = sum[r2+<span class="num">1</span>][c2+<span class="num">1</span>] - sum[r1][c2+<span class="num">1</span>]
           - sum[r2+<span class="num">1</span>][c1]   + sum[r1][c1];</pre></div>

<div class="callout callout-green">
  <strong>Classic problems:</strong> Range Sum Query (303), Range Sum 2D (304), Subarray Sum Equals K (560), Product of Array Except Self (238), Continuous Subarray Sum (523), Contiguous Array (525), Subarray Sums Divisible by K (974).
</div>

${revisionControls('algo-prefix')}

${quizHTML('algo-prefix', [
  { q: "Why does prefix sum + HashMap solve 'subarray sum equals K' in O(n)?", opts: ["HashMap sorts the array", "Brute force is already O(n)", "For each j, asking 'is there an i &lt; j with prefix[j] - prefix[i] = K' is equivalent to asking 'was prefix[j] - K ever seen?' — HashMap answers in O(1)", "It does not — that problem is O(n²)"], ans: 2, exp: "Any subarray arr[i+1..j] sums to prefix[j+1] - prefix[i]. So 'subarray ending at j sums to K' ↔ 'some earlier prefix equals prefix[j+1] - K'. HashMap of counts of prefix sums seen so far turns the inner loop into O(1) lookup, total O(n)." },
  { q: "2D prefix sum inclusion-exclusion — why subtract sum[i][j]?", opts: ["Arrays are 0-indexed", "The rectangle (0,0)-(i-1,j-1) is counted twice when adding row-above and col-left sub-rectangles, so subtract once to correct", "To prevent overflow", "It is not needed"], ans: 1, exp: "sum[i+1][j+1] = matrix[i][j] + sum[i][j+1] (full row above) + sum[i+1][j] (full col left). But the intersection sum[i][j] (top-left rectangle) is included in both. Subtract once to remove the double count — standard inclusion-exclusion." }
])}

`;

pages['algo-matrix'] = () => `
<div class="breadcrumb">Algorithms → <span>Matrix Traversal</span></div>
<div class="page-header">
  <div class="page-title">Matrix Traversal Pattern</div>
  <div class="page-subtitle">Treat a 2D grid as a graph where each cell is a node and neighbors are the 4 (or 8) adjacent cells. Apply DFS for connectivity, BFS for shortest path.</div>
</div>

<div class="memory-tip">
  <div class="mt-label">🎯 Trigger Keywords</div>
  <div class="mt-content">"grid", "matrix", "2D array", "islands", "flood fill", "connected regions", "shortest path in grid", "word search", "rotting oranges", "walls and gates". Any problem where the input is a 2D grid and you need to explore it.</div>
</div>

<div class="callout callout-blue">
  <strong>The Trick:</strong> A 2D grid IS a graph — each cell is a node, and its neighbors are the 4 adjacent cells (8 if diagonals count). Reuse your BFS or DFS template, just replace "for each neighbor of node" with "for each of the 4 directions from (r, c)". That one mental shift turns a family of seemingly different grid problems into graph problems you already know how to solve.
</div>

<div class="section-title">The Standard Setup</div>
<div class="code-block"><pre><span class="cm">// 4-directional (up, down, left, right)</span>
...
<span class="cm">// 8-directional (add diagonals)</span>
<span class="kw">int</span>[][] DIRS8 = {{-<span class="num">1</span>,-<span class="num">1</span>},{-<span class="num">1</span>,<span class="num">0</span>},{-<span class="num">1</span>,<span class="num">1</span>},{<span class="num">0</span>,-<span class="num">1</span>},
                 {<span class="num">0</span>,<span class="num">1</span>},{<span class="num">1</span>,-<span class="num">1</span>},{<span class="num">1</span>,<span class="num">0</span>},{<span class="num">1</span>,<span class="num">1</span>}};

<span class="cm">// Bounds check — write once, use everywhere</span>
<span class="kw">boolean</span> inBounds(<span class="kw">int</span> r, <span class="kw">int</span> c) {
    <span class="kw">return</span> r &gt;= <span class="num">0</span> &amp;&amp; r &lt; rows &amp;&amp; c &gt;= <span class="num">0</span> &amp;&amp; c &lt; cols;
}</pre></div>

<div class="section-title">DFS — Number of Islands (LeetCode 200)</div>
<div class="code-block"><pre><span class="kw">public int</span> numIslands(<span class="kw">char</span>[][] grid) {
    <span class="kw">int</span> count = <span class="num">0</span>;
    <span class="kw">for</span> (<span class="kw">int</span> r = <span class="num">0</span>; r &lt; grid.length; r++) {
        <span class="kw">for</span> (<span class="kw">int</span> c = <span class="num">0</span>; c &lt; grid[<span class="num">0</span>].length; c++) {
...
<div class="callout callout-amber">
  <strong>Visited tracking trade-off:</strong> (1) mutate grid in-place — zero extra space but destroys input, (2) separate <code>boolean[][] visited</code> — O(m×n) space, keeps input intact, (3) mutate then restore on the way back — zero space and preserves input but trickier. Mention all 3 and pick based on whether modifying input is acceptable.
</div>

<div class="section-title">BFS — Shortest Path in Grid</div>
<div class="code-block"><pre><span class="cm">// Rotting Oranges (LeetCode 994) — multi-source BFS</span>
<span class="kw">public int</span> orangesRotting(<span class="kw">int</span>[][] grid) {
    Queue&lt;<span class="kw">int</span>[]&gt; queue = <span class="kw">new</span> ArrayDeque&lt;&gt;();
    <span class="kw">int</span> fresh = <span class="num">0</span>;
    <span class="kw">for</span> (<span class="kw">int</span> r = <span class="num">0</span>; r &lt; grid.length; r++)
      <span class="kw">for</span> (<span class="kw">int</span> c = <span class="num">0</span>; c &lt; grid[<span class="num">0</span>].length; c++) {
          <span class="kw">if</span> (grid[r][c] == <span class="num">2</span>) queue.offer(<span class="kw">new int</span>[]{r, c});
          <span class="kw">else if</span> (grid[r][c] == <span class="num">1</span>) fresh++;
      }

    <span class="kw">int</span> minutes = <span class="num">0</span>;
    <span class="kw">while</span> (!queue.isEmpty() &amp;&amp; fresh &gt; <span class="num">0</span>) {
        <span class="kw">int</span> size = queue.size();
        <span class="kw">for</span> (<span class="kw">int</span> i = <span class="num">0</span>; i &lt; size; i++) {
            <span class="kw">int</span>[] p = queue.poll();
            <span class="kw">for</span> (<span class="kw">int</span>[] d : DIRS) {
                <span class="kw">int</span> nr = p[<span class="num">0</span>] + d[<span class="num">0</span>], nc = p[<span class="num">1</span>] + d[<span class="num">1</span>];
                <span class="kw">if</span> (nr &gt;= <span class="num">0</span> &amp;&amp; nr &lt; grid.length &amp;&amp;
                    nc &gt;= <span class="num">0</span> &amp;&amp; nc &lt; grid[<span class="num">0</span>].length &amp;&amp;
                    grid[nr][nc] == <span class="num">1</span>) {
                    grid[nr][nc] = <span class="num">2</span>;
                    fresh--;
    ...
    <span class="kw">return</span> fresh == <span class="num">0</span> ? minutes : -<span class="num">1</span>;
}</pre></div>

<div class="callout callout-green">
  <strong>Classic problems:</strong> Number of Islands (200), Rotting Oranges (994), Flood Fill (733), Word Search (79), Surrounded Regions (130), Pacific Atlantic Water Flow (417), Shortest Path in Binary Matrix (1091), Walls and Gates (286).
</div>

${revisionControls('algo-matrix')}

${quizHTML('algo-matrix', [
  { q: "When should you use BFS over DFS on a matrix?", opts: ["Always BFS", "Always DFS — it is faster", "When you need the shortest path in an unweighted grid. DFS explores deep and may find a longer path first; BFS guarantees the first time you reach a cell is the shortest distance.", "When the grid is larger than 100x100"], ans: 2, exp: "BFS explores in wavefronts — all cells at distance d are visited before any at distance d+1. That makes the first visit to a target cell the shortest distance. DFS goes deep, so the first path found is not necessarily shortest. Use DFS for connectivity/counting components, BFS for shortest path." },
  { q: "Multi-source BFS — why start with ALL source cells in the queue at once?", opts: ["To confuse the reader", "It does not matter", "Because all sources are at distance 0 simultaneously — seeding them together propagates the wavefront in parallel, giving correct shortest distance from ANY source to each cell in one BFS pass", "Single-source is always better"], ans: 2, exp: "If you need min distance from each cell to its nearest source (rotting oranges, walls and gates, 01 matrix), running BFS from each source is O((m×n)²). Multi-source BFS — enqueue all sources first, then BFS — gives the same result in O(m×n) total. The wavefront treats the combined source set as one super-source." }
])}
