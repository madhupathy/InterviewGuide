...
${quizHTML('algo-bfsdfs', [
  { q: "BFS vs DFS — when to prefer BFS?", opts: ["When finding shortest path in unweighted graph — BFS guarantees shortest path, DFS does not", "Always use BFS", "DFS can't handle cycles", "BFS uses less memory"], ans: 0, exp: "BFS explores level by level → first time you reach a node is the shortest path (in unweighted graphs). DFS may find a longer path first. BFS uses O(V) memory (queue), DFS uses O(depth) stack. Use DFS for: exhaustive search, backtracking, detecting cycles." },
  { q: "How to detect a cycle in a directed graph?", opts: ["DFS with 3 states: unvisited, in-progress (on current path), visited. Back edge to in-progress node = cycle", "Topological sort only", "Check if edge count > vertex count", "BFS only"], ans: 0, exp: "Color each node: WHITE (unvisited), GRAY (in current DFS path), BLACK (fully processed). If DFS encounters a GRAY node, there's a cycle on the current path. This is more precise than just 'visited' — it distinguishes back edges from cross edges." }
])}
...
<div class="section-title">The Core Trick</div>
