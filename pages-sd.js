...
${quizHTML('bb-caching', [
  { q: "Write-through vs write-behind cache — key trade-off?", opts: ["No difference", "Write-through: consistent but slower writes. Write-behind: fast writes but risk data loss if cache crashes before DB flush", "Write-behind is always better", "Write-through loses data"], ans: 1, exp: "Write-through: write to cache AND DB synchronously → data consistent but 2× write latency. Write-behind: write to cache, return to client, flush to DB async → fast but if cache crashes before flush = data lost. Use write-through for financial data, write-behind for analytics counters." },
  { q: "Redis Sentinel vs Redis Cluster — when to choose Cluster?", opts: ["Always use Cluster", "When dataset exceeds single node memory or you need horizontal write scaling — Cluster shards across 16,384 hash slots", "Cluster is simpler", "Sentinel is deprecated"], ans: 1, exp: "Sentinel: 1 primary + N replicas, auto-failover, all data on every node. Good for <25GB datasets. Cluster: data sharded across nodes, each shard has its own replicas. Needed when data exceeds single-node memory or write throughput exceeds single-primary capacity." }
])}
</div>
...
${quizHTML('bb-repl', [
  { q: "W=2, R=2, N=3 quorum — what does this guarantee?", opts: ["2 writes per second", "At least 1 node overlaps between write and read sets — guarantees reading the latest write (strong consistency)", "Faster reads", "No guarantees"], ans: 1, exp: "W+R > N means read and write sets overlap. With N=3, W=2, R=2: writes go to 2 of 3 nodes, reads check 2 of 3 nodes. At least 1 node has the latest write AND is read → strong consistency. Cassandra uses this with CONSISTENCY QUORUM." }
])}
...
${quizHTML('bb-acid', [
  { q: "MVCC (Multi-Version Concurrency Control) — what problem does it solve?", opts: ["Disk space", "Readers don't block writers and writers don't block readers — each transaction sees a consistent snapshot without locks", "Network latency", "Schema changes"], ans: 1, exp: "Without MVCC: reads must wait for writes to complete (or vice versa) via locks → poor concurrency. With MVCC (PostgreSQL, MySQL InnoDB): each transaction sees a snapshot of the DB at its start time. Writes create new versions, reads see the version valid at their snapshot time. No read locks needed." }
])}
...
${quizHTML('bb-idx', [
  { q: "Composite index (a, b, c) — which query does NOT use it?", opts: ["WHERE a=1", "WHERE a=1 AND b=2", "WHERE b=2 (skips leftmost column a — index unusable)", "WHERE a=1 AND b=2 AND c=3"], ans: 2, exp: "Leftmost prefix rule: composite index (a,b,c) supports queries that start from the left. WHERE a=1 ✅, WHERE a=1 AND b=2 ✅, WHERE a=1 AND b=2 AND c=3 ✅. But WHERE b=2 ❌ (skips 'a'). WHERE a=1 AND c=3 partially uses index (only 'a' part)." }
])}
</div>
...
${quizHTML('bb-apis', [
  { q: "REST vs gRPC — when to choose gRPC for microservices?", opts: ["Always use REST", "gRPC for internal services: binary Protocol Buffers, HTTP/2 multiplexing, native streaming — 5-10× less overhead than JSON/HTTP", "gRPC when browser support needed", "Only for Google services"], ans: 1, exp: "gRPC wins for internal service-to-service: Protocol Buffers ~6× smaller than JSON, HTTP/2 multiplexes streams on one connection, native bi-directional streaming. Use REST for public APIs and browser clients." },
  { q: "PUT vs PATCH — when to use PATCH?", opts: ["Always use PUT", "PATCH for partial updates — only sends changed fields. PUT replaces the entire resource (missing fields get nulled)", "They're identical", "PATCH is deprecated"], ans: 1, exp: "PUT: send complete object — server replaces everything. If you omit a field, it becomes null. PATCH: send only what changed — server merges with existing. Use PATCH for most updates (safer). Use PUT when intentionally replacing the full resource." }
])}
</div>
