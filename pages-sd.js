...
${quizHTML('sd-framework', [
  { q: "What's the first thing to do in a system design interview?", opts: ["Clarify functional and non-functional requirements for 5 minutes", "Choose the database", "Draw the architecture", "Estimate QPS"], ans: 0, exp: "Jumping straight to drawing is the #1 mistake. Spend 5 minutes asking: What does the system do? Who uses it? What scale? Read/write ratio? Consistency vs availability? Interviewers reward structured thinking." },
  ...
  { q: "When should you pick NoSQL over SQL?", opts: ["When you need massive horizontal scale, flexible schema, or very high write throughput", "For financial transactions", "When you need complex JOINs", "Always — NoSQL is faster"], ans: 0, exp: "Use SQL for: transactions, complex queries, relationships. Use NoSQL for: high write throughput (Cassandra), flexible/nested documents (MongoDB), key-value caching (Redis), time-series (InfluxDB). Most systems use both." },
  { q: "What does 'deep dive' mean in the SD interview framework?", opts: ["Draw all DB tables", "Rewrite the high-level design", "Discuss team structure", "Pick 2–3 critical components and explain design decisions, bottlenecks, and trade-offs in detail"], ans: 3, exp: "Deep dive = show expertise on specific components. Pick the hardest parts: How does the fan-out work? How do you handle hotspot users? How does the cache invalidation work? Walk through failure scenarios." },
  { q: "How do you estimate if a system needs sharding?", opts: ["When single DB can't handle the QPS or storage after read replicas and caching are exhausted", "Shard when you have 1M users", "When latency exceeds 1 second", "Always shard from day 1"], ans: 0, exp: "Scaling order: 1) Vertical scale-up, 2) Read replicas + caching (handles 90% of read-heavy problems), 3) Shard only if write QPS or storage exceeds single node. Sharding adds huge complexity — don't do it early." },
  { q: "What is the CAP theorem trade-off in practice?", opts: ["During a network partition: serve stale data (AP) or refuse requests (CP)", "Between latency and throughput", "Between SQL and NoSQL", "Choose between CPU and memory"], ans: 0, exp: "Since network partitions ALWAYS happen eventually, CAP means: CP systems (Zookeeper, etcd) stop accepting writes to maintain consistency. AP systems (Cassandra, DynamoDB) serve potentially stale data to stay available." },
  { q: "A system needs 99.99% availability. How many minutes of downtime/year is that?", opts: ["8.7 hours", "52 minutes", "~5 minutes", "~52 minutes"], ans: 3, exp: "99.99% ('four nines') = 0.01% downtime = 365.25 × 24 × 60 × 0.0001 = 52.6 minutes/year. 99.9% = 8.7 hours/year. 99.999% = 5.25 minutes/year. Know these by heart for the requirements discussion." },
  { q: "What's the difference between stateless and stateful services?", opts: ["Stateless requires a database", "Stateful is always faster", "Stateless: any server handles any request (horizontally scalable). Stateful: server holds session state (sticky sessions or external state store needed)", "No difference in practice"], ans: 2, exp: "Stateless services store all state externally (DB, Redis, JWT token). Any server can handle any request → simple horizontal scaling. Stateful services need sticky sessions or shared state (Redis, DB). Design microservices to be stateless." },
  { q: "What is a message queue used for in system design?", opts: ["Replacing the database", "Load balancing", "Speeding up synchronous API calls", "Decoupling services, absorbing traffic spikes, enabling async processing of slow operations"], ans: 3, exp: "Message queues (Kafka, SQS, RabbitMQ) decouple producers from consumers. Benefits: absorb traffic bursts (queue builds up instead of dropping requests), retry on failure, async processing (email, notifications, video transcoding), fan-out to multiple consumers." }
])}
...
${quizHTML('sd-estimation', [
  { q: "1 million requests/day is approximately how many QPS?", opts: ["~100 QPS", "~12 QPS", "~50 QPS", "~1,000 QPS"], ans: 1, exp: "1M ÷ 86,400 ≈ 11.6, round to ~12 QPS. Key formula: divide daily requests by 100,000 (10^5) for a quick mental estimate. So 100M/day ≈ 1,160 QPS." },
  { q: "Which operation is approximately 200,000× slower than an L1 cache hit?", opts: ["RAM", "Cross-region call", "SSD read", "L2 cache"], ans: 0, exp: "RAM access (~100ns) vs L1 cache (~0.5ns) = 200× slower. SSD is ~2,000,000× slower. Understanding this hierarchy explains why we cache aggressively." },
  ...
  { q: "A tweet is ~300 bytes. 300M tweets/day = how much storage?", opts: ["~90 MB", "~90 TB", "~90 GB", "~9 TB"], ans: 2, exp: "300M × 300 bytes = 90 GB/day for text only. Add metadata (~500B/tweet) = 300M × 800B = ~240 GB/day. Over 5 years text alone = ~440 TB. Always estimate text and media separately." },
  { q: "Peak traffic is typically how many times average?", opts: ["5–10× average", "50× average", "2× average", "Same as average"], ans: 0, exp: "System design rule: always plan for 5–10× peak vs average. Twitter at noon vs 3am is ~5× difference. Black Friday for e-commerce is 10×+. Always multiply your QPS estimate by 5 for capacity planning." }
,
  { q: "How many bytes is a typical user profile row in a database?", opts: ["~100 bytes", "~10 bytes", "~1 KB (name, email, bio, timestamps, preferences)", "~1 MB"], ans: 2, exp: "User profile: name(50B) + email(100B) + bio(500B) + created_at(8B) + settings JSON(200B) ≈ 1 KB. Always break down row sizes field by field in interviews — it shows you understand data modelling." },
  ...
  { q: "How many servers handle 350,000 reads/sec for a Twitter-like system?", opts: ["~35 servers (10K req/s per server, +50% buffer = 53 total)", "350 servers", "1 server", "3,500 servers"], ans: 0, exp: "Rule of thumb: 1 commodity server handles ~10K req/s for simple read APIs (stateless, cached responses). 350K ÷ 10K = 35 servers. Always add 50% capacity buffer for peak: 35 × 1.5 ≈ 53 servers. More for write-heavy or compute-heavy workloads." },
  { q: "Storage for 1 billion users with profile photos (100KB avg): how much?", opts: ["~100 PB", "~100 GB", "~100 TB", "~1 TB"], ans: 2, exp: "1B users × 100KB = 100 TB just for profile photos. With thumbnails at 3 sizes: 100TB × 3 = 300 TB. Over 5 years if 20% new users/year: ~500 TB total. This fits on S3/GCS with tiered storage. Always separate media storage from DB storage in estimates." }
])}
`;
...
${quizHTML('sd-patterns', [
  { q: "A social media feed has 100:1 read/write ratio. Which pattern?", opts: ["Read-Heavy", "Transactional", "Real-Time", "Write-Heavy"], ans: 0, exp: "Read-Heavy pattern. Solution: Redis cache in front of DB, read replicas, CDN for media. The mantra is 'don't make the DB do work reads can avoid'." },
  { q: "Payment system — most critical requirement?", opts: ["High throughput", "Low latency CDN", "Eventual consistency", "ACID transactions + idempotency keys"], ans: 3, exp: "Transactional pattern. Payments need ACID guarantees (SQL), idempotency keys to prevent duplicate charges, and row-level locking to prevent double-spend." },
  ...
  { q: "IoT sensors sending 1M events/sec. What pattern handles ingestion?", opts: ["Write-Heavy streaming ingestion with time-series DB", "REST API with synchronous DB writes", "Read-Heavy with heavy caching", "Transactional with 2PC"], ans: 0, exp: "Write-Heavy streaming. Ingest via Kafka (buffer spikes), write to time-series DB (InfluxDB/TimescaleDB optimised for sequential writes). Never write 1M/sec directly to a relational DB." },
  { q: "Global CDN is an example of which pattern?", opts: ["Write-Through", "Geo-Distributed / Edge pattern", "Read-Heavy Cache-Aside", "Async Processing"], ans: 1, exp: "Geo-Distributed / Edge pattern. Cache static assets at edge nodes close to users. Reduces origin server load, lowers latency (100ms → 5ms). CloudFront, Fastly, Cloudflare all implement this." },
  { q: "Which pattern uses fan-out on write?", opts: ["Event Sourcing", "Transactional", "Read-Heavy (fan-out on read)", "Social Graph / Feed (fan-out on write)"], ans: 3, exp: "Fan-out on write: when a celebrity posts, pre-compute and push to all followers' feed caches immediately. Expensive on write (1 post → 10M writes) but blazing fast reads. Twitter uses a hybrid: fan-out for regular users, fan-in for celebrities." },
  { q: "Circuit breaker pattern prevents what?", opts: ["SQL injection", "Data corruption", "Cascading failures — one slow service taking down all dependents", "Memory leaks"], ans: 2, exp: "Circuit breaker: after N failures, open the circuit → fail fast (return cached/default response immediately instead of waiting). Prevents thread pool exhaustion spreading a failure to all upstream services. Resilience4j, Hystrix implement this." },
  { q: "Event sourcing stores what instead of current state?", opts: ["Every event/change that happened — current state derived by replaying events", "Only the latest 100 events", "Compressed binary state", "Snapshots every minute"], ans: 0, exp: "Event sourcing: append OrderCreated, ItemAdded, PaymentMade events. Current state = replay. Benefits: full audit log, time travel, event replay for new projections. Used by Kafka Streams, Axon Framework, banking systems." }
])}
...
${quizHTML('bb-scaling', [
  { q: "What's the typical scaling order before resorting to database sharding?", opts: ["Shard immediately", "Add more RAM only", "Vertical scale → Cache (Redis) → Read replicas → CDN → Horizontal app servers → Async queues → Sharding", "Microservices first"], ans: 2, exp: "Each step adds ~10× capacity. Sharding is operationally the most expensive — most systems never need it. Start simple: bigger server → Redis cache → read replicas → CDN for static content → stateless app servers behind LB → async processing via queues." },
  { q: "1 Redis node handles approximately how many ops/sec?", opts: ["10,000", "1 million", "~100,000 ops/sec with sub-millisecond latency", "1,000"], ans: 2, exp: "Redis: ~100K ops/sec single-threaded. PostgreSQL: ~5K writes/sec. Single web server: ~10K req/sec. These numbers help you estimate when you need the next scaling step." }
])}
</div>
...
${quizHTML('bb-lb', [
  { q: "L7 load balancer advantage over L4?", opts: ["Lower cost", "Handles more connections", "Lower latency", "Routes by URL/headers enabling path-based routing, A/B testing, SSL termination"], ans: 3, exp: "L4 routes by IP/port only — fast but dumb. L7 understands HTTP: can route /api → service A, /static → CDN, add headers, terminate SSL. AWS ALB is L7; NLB is L4." },
  { q: "API Gateway vs Reverse Proxy — when do you need an API Gateway?", opts: ["Always", "When you need cross-cutting API concerns: auth, rate limiting per key, request transformation, versioning", "Only for microservices", "Never — reverse proxy does everything"], ans: 1, exp: "Reverse Proxy: LB + SSL + caching (simpler). API Gateway adds: authentication, per-user rate limiting, request/response transformation, API versioning, analytics. Use API Gateway when you have multiple client types (web, mobile, third-party) needing different API behaviors." }
])}
</div>
...
${quizHTML('bb-db', [
  { q: "When should you choose NoSQL over SQL?", opts: ["Always for new projects", "When you don't need transactions", "When you need massive horizontal scale, flexible schema, or a specific data model that doesn't fit tables", "NoSQL is always faster"], ans: 2, exp: "Default to SQL (PostgreSQL). Switch to NoSQL for: Cassandra (very high write throughput, multi-region), MongoDB (nested document structure), Redis (sub-ms caching), Neo4j (graph traversal). Most systems use BOTH — SQL for transactions, NoSQL for caching/search." }
])}
</div>
...
${quizHTML('bb-shard', [
  { q: "Consistent hashing main advantage over hash(key) % N?", opts: ["Better distribution", "Simpler implementation", "Faster computation", "Adding/removing a node only moves ~1/N keys instead of reshuffling almost everything"], ans: 3, exp: "With hash(key) % N: changing N causes nearly ALL keys to rehash — catastrophic cache miss storm. Consistent hashing: keys and nodes sit on a ring. Adding a node only affects keys between the new node and its predecessor (~1/N of total). Used by Cassandra, DynamoDB, CDNs." }
])}
</div>
...
${quizHTML('bb-repl', [
  { q: "W=2, R=2, N=3 quorum — what does this guarantee?", opts: ["Faster reads", "2 writes per second", "No guarantees", "At least 1 node overlaps between write and read sets — guarantees reading the latest write (strong consistency)"], ans: 3, exp: "W+R > N means read and write sets overlap. With N=3, W=2, R=2: writes go to 2 of 3 nodes, reads check 2 of 3 nodes. At least 1 node has the latest write AND is read → strong consistency. Cassandra uses this with CONSISTENCY QUORUM." }
])}
...
${quizHTML('bb-acid', [
  { q: "MVCC (Multi-Version Concurrency Control) — what problem does it solve?", opts: ["Disk space", "Schema changes", "Readers don't block writers and writers don't block readers — each transaction sees a consistent snapshot without locks", "Network latency"], ans: 2, exp: "Without MVCC: reads must wait for writes to complete (or vice versa) via locks → poor concurrency. With MVCC (PostgreSQL, MySQL InnoDB): each transaction sees a snapshot of the DB at its start time. Writes create new versions, reads see the version valid at their snapshot time. No read locks needed." }
])}
...
${quizHTML('bb-idx', [
  { q: "Composite index (a, b, c) — which query does NOT use it?", opts: ["WHERE b=2 (skips leftmost column a — index unusable)", "WHERE a=1 AND b=2", "WHERE a=1", "WHERE a=1 AND b=2 AND c=3"], ans: 0, exp: "Leftmost prefix rule: composite index (a,b,c) supports queries that start from the left. WHERE a=1 ✅, WHERE a=1 AND b=2 ✅, WHERE a=1 AND b=2 AND c=3 ✅. But WHERE b=2 ❌ (skips 'a'). WHERE a=1 AND c=3 partially uses index (only 'a' part)." }
])}
</div>
...
${quizHTML('bb-apis', [
  { q: "REST vs gRPC — when to choose gRPC for microservices?", opts: ["Always use REST", "gRPC when browser support needed", "gRPC for internal services: binary Protocol Buffers, HTTP/2 multiplexing, native streaming — 5-10× less overhead than JSON/HTTP", "Only for Google services"], ans: 2, exp: "gRPC wins for internal service-to-service: Protocol Buffers ~6× smaller than JSON, HTTP/2 multiplexes streams on one connection, native bi-directional streaming. Use REST for public APIs and browser clients." },
  { q: "PUT vs PATCH — when to use PATCH?", opts: ["PATCH is deprecated", "They're identical", "Always use PUT", "PATCH for partial updates — only sends changed fields. PUT replaces the entire resource (missing fields get nulled)"], ans: 3, exp: "PUT: send complete object — server replaces everything. If you omit a field, it becomes null. PATCH: send only what changed — server merges with existing. Use PATCH for most updates (safer). Use PUT when intentionally replacing the full resource." }
])}
</div>
...
${quizHTML('sd-auth', [
  { q: "JWT vs Session tokens — key difference?", opts: ["Sessions are more secure", "JWT is always encrypted", "JWT is stateless (server holds no state); sessions require server-side storage", "No difference"], ans: 2, exp: "Session: server stores session data (Redis/DB), client gets opaque ID. JWT: all data in the token, server only verifies signature — no storage. JWT enables stateless horizontal scaling but can't be revoked without a blocklist." },
  { q: "OAuth2 is for?", opts: ["Authenticating users (proving identity)", "Authorisation — granting third-party apps limited access to your resources", "Encrypting passwords", "Hashing tokens"], ans: 1, exp: "OAuth2 = Authorisation (what you can access). OpenID Connect (OIDC) = Authentication on top of OAuth2 (who you are). When you 'Login with Google', OAuth2 grants access, OIDC provides identity (id_token with user info)." },
  { q: "When should you use API Keys over JWT?", opts: ["Server-to-server integrations, third-party developer access, rate-limiting by client", "Browser-based SPAs", "Never — JWT is always better", "Mobile apps"], ans: 0, exp: "API Keys: simple, long-lived, good for server-to-server or third-party developers. JWT: better for user sessions (short-lived, carries claims). API Keys are easy to audit, rotate, and rate-limit per key." },
  { q: "What is the purpose of a refresh token?", opts: ["Refresh the UI", "Reset password", "Reload user profile", "Get new short-lived access tokens without re-authentication when access token expires"], ans: 3, exp: "Access tokens are short-lived (15min–1hr) to limit damage if stolen. Refresh tokens are long-lived (days/weeks), stored securely (httpOnly cookie), used ONLY to get new access tokens. If refresh token is stolen, revoke it in the DB." },
  { q: "PKCE (Proof Key for Code Exchange) prevents what attack?", opts: ["CSRF attacks", "JWT forgery", "Authorization code interception in mobile/SPA OAuth flows", "SQL injection"], ans: 2, exp: "In public clients (mobile apps, SPAs), the authorization code could be intercepted. PKCE: generate random code_verifier → hash it (code_challenge) → send hash with auth request. Only the original app that sent the hash can exchange the code." },
  { q: "Where should JWTs be stored in a browser?", opts: ["httpOnly cookies (prevents XSS access) — NOT localStorage", "window.token global variable", "localStorage (most convenient)", "sessionStorage"], ans: 0, exp: "localStorage is accessible via JavaScript — XSS attack can steal it. httpOnly cookies can't be accessed by JS. Pair with SameSite=Strict/Lax to prevent CSRF. This is the recommended approach for auth tokens." },
  { q: "What is mTLS and when is it used?", opts: ["Mutual TLS — both client AND server present certificates, verifying each other's identity", "Mobile TLS protocol", "A compression algorithm", "Multi-factor TLS"], ans: 0, exp: "Normal TLS: only server proves identity (certificate). mTLS: both sides present certificates. Used for service-to-service auth in microservices (Istio implements this). Prevents man-in-the-middle between internal services." },
  { q: "Rate limiting by user vs by IP — which is harder to bypass?", opts: ["By authenticated user ID (harder to bypass than IP, which can be spoofed/rotated)", "By IP (harder)", "Both equally hard", "Neither can be bypassed"], ans: 0, exp: "IP-based limiting: attackers can use proxies, rotating IPs, botnets. User-ID based: requires authentication, much harder to get millions of accounts. Best: both layers — IP rate limiting for unauthenticated endpoints, user-ID for authenticated." },
  { q: "SSO (Single Sign-On) uses which protocols?", opts: ["Only JWT", "SAML 2.0 (enterprise/XML) or OIDC (modern/JSON) — both enable one login for multiple apps", "Only OAuth2", "HTTP Basic Auth"], ans: 1, exp: "SAML 2.0: XML-based, enterprise-focused (Okta, Active Directory). OIDC: JSON/JWT-based, modern web/mobile (Google, GitHub). Both allow one authentication session to grant access to multiple applications without re-login." }
])}
`;
...
${quizHTML('sd-ratelimit', [
  ...
  { q: "Token bucket vs sliding window log — main difference?", opts: ["Token bucket stores more data", "Token bucket: O(1) memory, allows bursts. Sliding window log: exact, O(n) memory (stores every timestamp)", "No real difference", "Window log is simpler"], ans: 1, exp: "Token bucket: 2 Redis keys, constant memory, allows burst up to bucket size. Sliding window log: one sorted set entry per request — O(n) memory. At 10K req/user/hour, log uses 10K entries vs bucket's 2 counters." },
  { q: "The 'boundary problem' affects which algorithm?", opts: ["Sliding window counter", "Token bucket", "Fixed window counter — allows 2× the limit at window boundaries", "Leaky bucket"], ans: 2, exp: "Fixed window: 100 req limit. User sends 100 at 11:59:59, then 100 more at 12:00:01. Both windows see 100 (within limit), but 200 requests happened in 2 seconds. Sliding window algorithms eliminate this." },
  { q: "Where should you implement rate limiting for a microservices system?", opts: ["Only in the application code", "At the DNS level", "In the database", "API Gateway for global limits + per-service middleware for fine-grained limits + Redis for distributed state"], ans: 3, exp: "Multi-layer approach: 1) API Gateway (Kong/AWS API GW) — first line of defense, per-API-key limits. 2) Service middleware — per-user, per-feature limits. 3) Redis — shared distributed counter. Defense in depth." },
  { q: "What HTTP status code should rate-limited requests return?", opts: ["429 Too Many Requests", "400 Bad Request", "401 Unauthorized", "503 Service Unavailable"], ans: 0, exp: "429 Too Many Requests is the correct status. Include Retry-After header to tell the client when to try again. Also include X-RateLimit-Limit, X-RateLimit-Remaining, X-RateLimit-Reset headers for transparency." },
  { q: "Sliding window counter is ~98% accurate. What causes the 2% error?", opts: ["It approximates the previous window's contribution using a weighted ratio, not exact timestamps", "Network latency", "Integer overflow", "Hash collisions in Redis"], ans: 0, exp: "Formula: count = prev_window_count × (1 - elapsed_in_current_window / window_size) + current_count. The approximation assumes requests were uniformly distributed in the previous window — which introduces a small error." },
  { q: "How do you handle rate limit for a celebrity user with millions of API calls?", opts: ["Move them to a dedicated server", "Give them a higher tier limit or whitelist, implement per-endpoint limits separately", "Block them entirely", "Use the same limit as everyone else"], ans: 1, exp: "Tiered limits: free=100/day, pro=10K/day, enterprise=unlimited (fair use). Whitelist internal services and trusted partners. For celebrity users in social apps, fan-out is the bigger problem — use async queues, not stricter rate limits." },
  { q: "What is the leaky bucket algorithm good for?", opts: ["Smoothing output rate — requests are processed at a constant rate regardless of burst input", "Highest accuracy", "Allowing burst traffic", "Lowest memory usage"], ans: 0, exp: "Leaky bucket: input flows in at variable rate, output leaks at fixed rate. Useful when you want smooth, predictable output (e.g., sending SMS/emails at a fixed rate to avoid overwhelming downstream). Unlike token bucket, NO bursting allowed." },
  { q: "How do you prevent rate limit bypass via multiple accounts?", opts: ["Use stronger encryption", "Device fingerprinting, IP-based secondary limits, anomaly detection, CAPTCHA on account creation", "Only allow one account per email", "You can't prevent it"], ans: 1, exp: "Defense layers: 1) IP rate limiting as secondary control, 2) Device fingerprinting, 3) ML-based anomaly detection (100 new accounts from same subnet), 4) Phone verification on signup, 5) Behavioral analysis (too-regular request patterns)." }
])}
`;
...
${quizHTML('sd-lru', [
  { q: "Why use a Doubly Linked List (not singly) for LRU?", opts: ["Singly linked doesn't support null", "Memory efficiency", "O(1) removal requires both prev and next pointers — singly linked needs O(n) traversal to find prev", "Doubly linked is simpler"], ans: 2, exp: "To remove a node: update prev.next and next.prev. With singly linked, you don't know prev without traversing from head — O(n). Doubly linked gives O(1) removal anywhere in the list." },
  { q: "LRU cache get() and put() should be?", opts: ["O(log n)", "O(n log n)", "O(1)", "O(n)"], ans: 2, exp: "HashMap: O(1) key lookup. Doubly linked list: O(1) insert/remove with node reference from map. Combined: both O(1). This is exactly why we combine these two data structures." },
  ...
  { q: "LinkedHashMap in Java with accessOrder=true does what?", opts: ["Sorts by insertion time", "Sorts by key alphabetically", "Compresses old entries", "Maintains LRU order automatically — most recently accessed entry moves to tail"], ans: 3, exp: "new LinkedHashMap<>(cap, 0.75f, true) — the third param 'accessOrder=true' reorders entries on every get()/put(). Combined with overriding removeEldestEntry(), you get a working LRU cache in 10 lines." },
  { q: "LRU cache with capacity 3. Operations: put(1), put(2), put(3), get(1), put(4). What's evicted?", opts: ["Key 4", "Key 3", "Key 1 (oldest)", "Key 2 (LRU after get(1) made 1 recent)"], ans: 3, exp: "After put(1,2,3): order = [3,2,1] (3 most recent). get(1) makes 1 most recent: [1,3,2]. put(4) evicts LRU = key 2. Final cache: {1,3,4}. Always trace through the doubly linked list to verify." },
  { q: "What is a cache stampede and how to fix it for LRU?", opts: ["When cache miss triggers many simultaneous DB calls for same key — fix: mutex lock or probabilistic early expiry", "When LRU evicts too aggressively", "When two keys hash to same bucket", "When cache gets too full"], ans: 0, exp: "Stampede: popular key expires, 1000 requests simultaneously miss cache → 1000 DB queries. Fix: 1) Mutex: first thread queries DB, others wait, 2) Probabilistic early expiry (expire before TTL with some probability), 3) Background refresh before expiry." },
  { q: "Caffeine vs Guava cache for Java local caching?", opts: ["Guava is always better", "They are identical", "Caffeine is superior: W-TinyLFU eviction (better hit rate than LRU), near-optimal, faster, active development", "Guava has more features"], ans: 2, exp: "Caffeine uses W-TinyLFU (Window Tiny LFU) — a hybrid algorithm that tracks both frequency AND recency, outperforming pure LRU on most real workloads. It's the recommended replacement for Guava cache and is used by Spring Boot's default cache." },
  { q: "For a distributed cache, what's the consistency trade-off?", opts: ["No trade-off exists", "Cache is always consistent", "Must use strong consistency always", "Cache may serve stale data (AP) — choose TTL to balance freshness vs DB load"], ans: 3, exp: "Distributed cache is eventually consistent by design. TTL controls staleness: short TTL = fresher data, more DB hits; long TTL = stale data risk, fewer DB hits. For user profiles: 5 min TTL is fine. For inventory/prices: shorter TTL or event-based invalidation." },
  { q: "LRU cache size: 1M entries with 1KB average value. How much RAM?", opts: ["~1 GB + overhead", "~1 TB", "~1 MB", "~100 MB"], ans: 0, exp: "1M × 1KB = 1GB for values + HashMap overhead (~50 bytes/entry × 1M = 50MB) + DLL node overhead (~32 bytes × 1M = 32MB) ≈ 1.1GB total. Redis adds ~65 bytes per key overhead. Always estimate cache RAM requirements during system design." }
])}
`;
...
${quizHTML('sd-msgqueue', [
  { q: "Key difference between Kafka and RabbitMQ message retention?", opts: ["No difference", "Both delete immediately", "Kafka persists messages configurable forever allowing replay; RabbitMQ deletes on acknowledgment", "RabbitMQ keeps messages longer"], ans: 2, exp: "RabbitMQ: message is gone once a consumer acknowledges it — designed for task queues. Kafka: messages persist by time or size policy (days, forever). Multiple consumer groups can read the same message independently. Replay past events anytime." },
  { q: "Why is Kafka throughput orders of magnitude higher than RabbitMQ?", opts: ["Kafka uses UDP", "Sequential disk writes + OS page cache + zero-copy sendfile + producer batching", "RabbitMQ limits connections", "Kafka uses more memory"], ans: 1, exp: "Kafka's speed: 1) Sequential log appends (OS page cache ≈ RAM speed), 2) sendfile() zero-copy skips user-space, 3) Producer batching compresses 10-100 messages together, 4) Partitions allow N-parallel consumers. RabbitMQ maintains per-message ACK state — much more overhead." },
  { q: "Consumer groups in Kafka mean?", opts: ["Groups delete messages faster", "Multiple independent groups each read ALL messages — billing and analytics can both consume orders independently", "Groups share a single offset", "Only one consumer per topic"], ans: 1, exp: "Each consumer group maintains its own offset per partition. Add a new consumer group and it starts reading from the beginning (or any offset). This is Kafka's killer feature for event streaming — fanout to N systems with zero coupling." },
  { q: "What is a Dead Letter Queue in RabbitMQ?", opts: ["A backup queue", "A queue for system logs", "A queue receiving messages that failed processing — prevents poison messages blocking the main queue", "RabbitMQ's internal queue"], ans: 2, exp: "DLQ: when a message is rejected (consumer throws exception, TTL expires, max retries reached), it routes to the Dead Letter Exchange → Dead Letter Queue. Ops team can inspect, fix, and replay from DLQ. Critical for production reliability." },
  { q: "Kafka ordering guarantee — what's the caveat?", opts: ["Ordering guaranteed per partition only — not globally across all partitions", "Ordered by timestamp always", "Global ordering across all partitions", "No ordering at all"], ans: 0, exp: "Within a partition: strict offset order guaranteed. Across partitions: no global order. Solution: use a partition key (userId, orderId) — all messages for the same key go to the same partition, giving per-entity ordering. This is intentional — it enables parallelism." },
  { q: "Idempotent consumers in Kafka — why needed?", opts: ["At-least-once delivery can duplicate messages on failure — consumers must handle duplicate processing safely", "Only for multi-partition topics", "For better performance", "Required by Kafka protocol"], ans: 0, exp: "With at-least-once: consumer processes message, commits offset, but crashes before commit → message reprocessed. Make consumers idempotent: check if already processed (dedup by event ID), use DB upserts. Exactly-once Kafka transactions are available but add overhead." },
  { q: "When would you choose RabbitMQ over Kafka for a payment system?", opts: ["For task-queue style: each payment processed once, smart routing by payment type, simpler ops, no need for replay", "When scale exceeds 1M/sec", "Never — always use Kafka", "When using Python only"], ans: 0, exp: "Payment processing is often task-queue style: each message processed once, ACK on success, NACK (+ DLQ) on failure. RabbitMQ routing lets you send domestic vs international payments to different queues with different consumers. Much simpler than Kafka for moderate throughput." },
  { q: "What is Change Data Capture (CDC) with Kafka?", opts: ["A Kafka compression method", "Consumer group management", "A backup strategy", "Capturing every DB change as a Kafka event — Debezium reads MySQL/Postgres WAL and publishes changes as events"], ans: 3, exp: "CDC with Debezium: reads database WAL (Write-Ahead Log), publishes INSERT/UPDATE/DELETE as Kafka events. Use cases: sync DB to Elasticsearch, invalidate caches on DB change, audit trail, microservice event sourcing. Zero impact on application code." },
  { q: "Kafka partition count — why does it matter for scaling?", opts: ["Only affects storage", "Partitions = max parallel consumers; 10 partitions = max 10 consumers in a group can process in parallel", "Partitions are unlimited", "More partitions = slower"], ans: 1, exp: "Each partition assigned to at most one consumer per group. 3 partitions + 10 consumers = 3 active, 7 idle. To scale to 10 parallel consumers: need 10 partitions. Partitions can be increased (but causes rebalancing) — plan partition count for expected peak parallelism." }
])}
`;
...
${quizHTML('sd-tradeoffs', [
  { q: "Which consistency model should you use for a bank transfer?", opts: ["Depends on bank size", "Strong consistency — stale balance = double spend", "Eventual consistency — faster", "Either works"], ans: 1, exp: "Banking requires strong consistency. If two servers have different balances, a user could spend money they don't have or funds could be deducted twice. Use CP systems (Postgres with serializable isolation) and accept the latency cost." },
  { q: "REST vs gRPC — when to choose gRPC for microservices?", opts: ["gRPC only for Google services", "Always use REST", "gRPC when browser support needed", "gRPC for internal services: binary protocol, typed schema, streaming, 5-10x lower overhead than JSON/HTTP"], ans: 3, exp: "gRPC wins for internal microservices: Protocol Buffers are ~6x smaller than JSON, HTTP/2 multiplexes multiple streams on one connection, native bi-directional streaming. Use REST for public APIs and browser clients where gRPC-Web adds complexity." },
  ...
  { q: "Batch vs stream processing — which to use for fraud detection?", opts: ["Both are equally good", "Stream processing — analyze each transaction in real-time as it happens", "Batch — process daily transactions overnight", "Batch with 5-minute intervals"], ans: 1, exp: "Fraud detection requires stream processing: analyze each transaction the moment it occurs, flag and block before the fraudster gets away. Batch processing (nightly reports) would be useless — the fraudulent transactions have already completed." }
])}
`;
...
${quizHTML('sd-eventing', [
  { q: "What are the three pillars of observability?", opts: ["Availability, Performance, Security", "Logs (what happened), Metrics (aggregated numbers), Traces (request journey)", "Latency, Throughput, Errors", "CPU, Memory, Network"], ans: 1, exp: "Logs: timestamped events. Metrics: numeric aggregates (QPS, error rate). Traces: distributed request journey across services with timing. All three needed for complete debugging. Correlate them with a shared traceId in log fields." },
  { q: "Fluent Bit vs FluentD — which to use as a DaemonSet on K8s nodes?", opts: ["Neither — use Prometheus instead", "Fluent Bit — ~1MB memory vs FluentD ~40MB, designed for edge collection", "FluentD — more plugins", "Both are identical"], ans: 1, exp: "Fluent Bit: C-only, ~1MB memory, designed as lightweight forwarder. Perfect as DaemonSet. FluentD: Ruby/C, ~40MB, better for complex aggregation and routing. Common pattern: Fluent Bit (node DaemonSet) → FluentD (aggregator) → multiple destinations." },
  { q: "OpenTelemetry Collector main benefit?", opts: ["Vendor-neutral hub: apps instrument once, Collector fans out to Prometheus, Jaeger, Loki simultaneously", "Only for traces", "Replaces Prometheus", "Requires Kubernetes"], ans: 0, exp: "OTel Collector receives OTLP signals from apps → processes (batch, filter, enrich) → exports to multiple backends simultaneously. Change backends without touching app code. One SDK, any observability stack." },
  { q: "Prometheus scrape model — pull vs push?", opts: ["Push — apps send metrics", "Websocket streaming", "Pull — Prometheus scrapes /metrics endpoints at intervals", "Both equally"], ans: 2, exp: "Prometheus pulls: scrapes configured /metrics endpoints every N seconds. Benefits: easy to detect down services (scrape fails = down), no push config needed in apps, centrally controlled interval. Use Pushgateway for batch jobs that can't be scraped." },
  { q: "Structured logging main advantage over plain text?", opts: ["Required by Kubernetes", "JSON fields queryable by any key — userId, traceId, amount — enables filtering, alerting, correlation", "Faster to write", "Smaller file size"], ans: 1, exp: "Plain text: grep is the only tool. Structured JSON in Loki: {app='api'} | json | userId=12345 instantly finds all events for that user. Including traceId in every log line lets you jump from a log event directly to the distributed trace in Jaeger." },
  { q: "RED method stands for?", opts: ["Reliability, Efficiency, Durability", "Rate (req/s), Errors (error rate), Duration (latency p50/p95/p99)", "Response, Endpoint, Debugging", "Requests, Execution, Data"], ans: 1, exp: "RED = golden signals for measuring service health. Rate: requests per second. Errors: fraction that fail. Duration: how long they take. USE method (Utilization, Saturation, Errors) is for infrastructure resources. Dashboard with both covers 90% of incidents." },
  { q: "How do you correlate logs, metrics, and traces?", opts: ["Timestamp matching only", "Use a single tool only", "Not possible across different tools", "Inject traceId into every log field + exemplar links in Prometheus + Grafana unified view"], ans: 3, exp: "Correlation: 1) OTel SDK injects traceId into logs automatically, 2) Prometheus exemplars link metric data points to traces, 3) Grafana shows all three — click a spike → see logs for that window → jump to the specific trace. The traceId is the glue." },
  { q: "What is a Prometheus alert rule's 'for' duration used for?", opts: ["Must be in alert condition continuously for this duration before firing — prevents flapping on brief spikes", "Alert frequency", "How long to collect data before alerting", "When to auto-resolve"], ans: 0, exp: "Without 'for: 5m': a single-second spike fires the alert immediately. With 'for: 5m': condition must be true continuously for 5 minutes before PagerDuty rings. Prevents false alerts from brief traffic spikes. Set 'for' based on how long the problem must persist to be real." },
  { q: "Sidecar pattern for logging — what problem does it solve?", opts: ["Decouples app from log shipping — app writes to stdout, sidecar handles collection, parsing, forwarding without app changes", "Network routing", "App performance", "Security isolation"], ans: 0, exp: "Sidecar: Fluent Bit container in same pod as app. App writes logs to stdout/files, sidecar reads and ships to Loki. Apps don't need logging SDKs or network config. Change log destination by updating sidecar config only. Used by Istio (Envoy), FluentD, Vault agent." }
])}
`;
...
${quizHTML('sd-specdriven', [
  { q: "What problem does RAG solve?", opts: ["Replaces the model entirely", "Makes LLMs faster", "Gives LLMs access to up-to-date private knowledge without expensive fine-tuning — retrieve at query time", "Reduces hallucinations to zero"], ans: 2, exp: "RAG: at query time, embed the question → vector similarity search → inject top-K relevant chunks into the prompt → LLM answers using your data. No retraining needed. Knowledge can be updated by re-indexing documents. Cheaper than fine-tuning for factual knowledge." },
  { q: "What is MCP (Model Context Protocol)?", opts: ["Open protocol for AI models to connect to external tools, databases, and services via a standard interface", "A training methodology", "A compression standard", "A meta-learning algorithm"], ans: 0, exp: "MCP standardises how AI applications connect to capabilities — like USB-C for AI integrations. MCP servers expose tools (callable functions), resources (readable data), and prompts. Hosts like Claude Desktop, VS Code connect to any MCP server using the same protocol." },
  { q: "Chunking overlap (e.g., 10-20%) in RAG — why?", opts: ["Improves embedding speed", "Required by vector databases", "Preserves context at chunk boundaries — a sentence split across two chunks stays coherent", "Reduces storage"], ans: 2, exp: "Without overlap: a concept split across chunk boundary loses coherence — the embedding doesn't capture the full idea. With 50-token overlap: the tail of chunk N is repeated at the start of chunk N+1. Slightly more storage but much better retrieval quality for boundary queries." },
  { q: "pgvector vs Pinecone — when to choose pgvector?", opts: ["Pinecone only supports Python", "pgvector is always faster", "pgvector when you already use Postgres and have under ~10M vectors — no new infra, SQL queries work", "Always use Pinecone"], ans: 2, exp: "pgvector: PostgreSQL extension, add vector type to existing tables, combine with SQL filters, no new service. Perfect for smaller RAG systems already on Postgres. Pinecone/Qdrant for production scale (100M+ vectors), advanced filtering, or dedicated vector workloads." },
  { q: "ReAct pattern in agents — what does it stand for?", opts: ["Reactive programming", "Real-time Actions", "Reasoning + Acting — LLM alternates between thinking about what to do and calling tools to do it", "REST and CRUD"], ans: 2, exp: "ReAct loop: Thought (what do I need?) → Action (call tool) → Observation (tool result) → Thought → Action → ... until final answer. The model explicitly reasons about each step before acting. More reliable than pure end-to-end generation for multi-step tasks." },
  { q: "Semantic chunking advantage over fixed-size chunking?", opts: ["Uses less memory", "Splits where topic changes (semantic boundary) rather than at arbitrary token count — preserves coherent ideas in each chunk", "Always faster", "Required for OpenAI embeddings"], ans: 1, exp: "Fixed-size: may cut mid-sentence or mid-concept. Semantic: embed sentences, measure similarity to neighbours, split when similarity drops significantly — topic boundary. Better retrieval quality especially for documents that mix multiple topics. Higher compute cost." },
  { q: "Multi-agent system key design challenge?", opts: ["Preventing infinite loops, handling failures gracefully, coordinating state between agents without conflicts", "Choosing programming language", "Setting up the database", "Writing prompt templates"], ans: 0, exp: "Multi-agent challenges: 1) Infinite loops (set max_iterations), 2) Agent failures (retry/escalate/fallback), 3) State consistency (which agent's memory is authoritative?), 4) Cost control (each LLM call costs money), 5) Debugging (hard to trace multi-agent reasoning). Add circuit breakers and human checkpoints for high-stakes actions." }
])}
`;
