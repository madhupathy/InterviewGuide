...
${quizHTML('k8s-core', [
  { q: "What stores cluster state in Kubernetes?", opts: ["kube-apiserver", "Scheduler", "etcd — distributed KV store, single source of truth", "kubelet"], ans: 2, exp: "etcd is the brain of Kubernetes. All cluster state (pods, services, deployments, secrets) is stored in etcd. The API server is the only component that reads/writes etcd. Always back up etcd — losing it = losing the cluster." },
  { q: "Deployment vs StatefulSet — when to use each?", opts: ["Deployment: stateless apps (any pod replaceable). StatefulSet: stateful (stable DNS names, stable storage, ordered ops)", "StatefulSet has more replicas", "StatefulSet can't scale", "No difference"], ans: 0, exp: "Deployment: pods are interchangeable, random names, no stable storage. StatefulSet: pods have predictable names (pod-0, pod-1), stable DNS (pod-0.service.ns.svc.cluster.local), each gets its own PVC. Use for: Postgres, Kafka, Elasticsearch, Zookeeper." },
  { q: "What does kube-proxy do on each node?", opts: ["Manages iptables/IPVS rules implementing Service VIP → Pod IP routing", "Acts as ingress controller", "Proxies traffic from outside the cluster", "Manages container networking"], ans: 0, exp: "kube-proxy: runs on every node, watches Services and Endpoints, programs iptables (or IPVS) rules so traffic to ClusterIP:Port gets load-balanced to one of the healthy Pod IPs. Not an actual proxy in modern mode — just manages routing rules." },
  { q: "What is a Kubernetes Namespace?", opts: ["A container registry", "A node group", "A DNS domain", "Virtual cluster within a cluster — provides isolation of resources, RBAC scope, and resource quotas"], ans: 3, exp: "Namespaces partition cluster resources. Same name allowed in different namespaces. DNS: service.namespace.svc.cluster.local. Apply: RBAC permissions, ResourceQuotas, LimitRanges per namespace. Use namespaces for: team isolation, environment separation (dev/staging/prod)." },
  { q: "ConfigMap vs Secret — what's the security difference?", opts: ["Secrets encrypt automatically", "No difference", "ConfigMaps support binary data", "Secrets are base64-encoded and can be encrypted at rest in etcd; ConfigMaps are plain text"], ans: 3, exp: "ConfigMap: plain text key-value pairs for non-sensitive config. Secret: base64-encoded (NOT encrypted by default!). Enable encryption at rest with --encryption-provider-config in API server. Also: use external secret management (Vault, AWS Secrets Manager) for production." },
  { q: "What happens to pods when a Deployment is updated?", opts: ["All pods restart simultaneously", "Pods are deleted and recreated", "Nothing until manually triggered", "Rolling update: new pods created gradually, old pods terminated — controlled by maxSurge and maxUnavailable"], ans: 3, exp: "Rolling update (default): maxSurge=1 allows 1 extra pod during update; maxUnavailable=0 ensures no capacity loss. Recreate strategy: all old pods deleted first (downtime). Blue/Green: separate deployment, switch Service selector. Canary: small percentage to new version." },
  { q: "What is a DaemonSet used for?", opts: ["Running pods on schedule", "Managing StatefulSets", "Running one pod on EVERY node — used for node-level agents like log collectors, monitoring, CNI plugins", "Running exactly one replica"], ans: 2, exp: "DaemonSet ensures one pod runs on each node (or a subset). Automatically adds pods when nodes join, removes when nodes leave. Used for: fluentd (log collection), Prometheus node-exporter, datadog agent, Calico/Flannel CNI. Not suitable for application workloads." },
  { q: "How does Kubernetes service discovery work?", opts: ["Manual /etc/hosts", "DNS: kube-dns/CoreDNS provides service-name.namespace.svc.cluster.local resolving to ClusterIP", "etcd lookup", "Static IP configuration"], ans: 1, exp: "CoreDNS runs as a Deployment in kube-system. Every Pod's /etc/resolv.conf points to CoreDNS. Accessing 'my-service' resolves to my-service.default.svc.cluster.local → ClusterIP → kube-proxy routes to Pod. Cross-namespace: my-service.other-namespace.svc.cluster.local." },
  { q: "What causes Pending pod status?", opts: ["Application crash", "Container starting", "Image not found", "Unschedulable: insufficient CPU/memory on nodes, node selector mismatch, PVC not bound, or taint not tolerated"], ans: 3, exp: "Pending = scheduler can't find a suitable node. Causes: 1) Insufficient CPU/memory (requests too high), 2) NodeSelector/Affinity not matching any node, 3) PVC pending (no matching PV/StorageClass), 4) Node has taint not tolerated by pod. kubectl describe pod shows Events with reason." }
])}`;
...
${quizHTML('k8s-troubleshoot', [
  { q: "CrashLoopBackOff with exit code 137 — what happened?", opts: ["App error in code", "Image not found", "Liveness probe failed", "OOMKilled — container exceeded memory limit and was killed by Linux kernel"], ans: 3, exp: "Exit code 137 = 128 + SIGKILL(9). The kernel killed the container because it exceeded its memory limit (OOMKilled). Solution: 1) Increase memory limit, 2) Find memory leak (take heap dump before OOM), 3) Use VPA to right-size automatically." },
  { q: "Service returns 'connection refused' — first diagnostic step?", opts: ["Restart all pods", "Check DNS", "kubectl get endpoints <service> — if empty, selector doesn't match pod labels", "Restart kube-proxy"], ans: 2, exp: "Empty endpoints is the #1 cause. Service selector must match pod labels exactly (case-sensitive). Compare: kubectl get svc -o yaml (spec.selector) vs kubectl get pods --show-labels. Also check: are pods Running and Ready? Is targetPort matching container port?" },
  { q: "ImagePullBackOff — what to check first?", opts: ["Node disk space", "Network policies", "Image name/tag exists in registry, imagePullSecrets configured for private registries, registry reachable from node", "Resource limits"], ans: 2, exp: "Steps: 1) Verify image name and tag exist (typo is most common), 2) For private registry: create docker-registry Secret and add imagePullSecrets to Pod spec or ServiceAccount, 3) Check node network connectivity to registry, 4) kubectl describe pod → Events show exact error." },
  { q: "Pod is Running but application returns 503 — likely cause?", opts: ["Pod crashed", "Readiness probe failing — pod is running but not ready, removed from Service endpoints", "DNS issue", "Service misconfiguration"], ans: 1, exp: "503 from Service = no Ready endpoints. Pod Running ≠ Ready. Readiness probe failure removes pod from Service. Check: kubectl describe pod → 'Readiness probe failed'. Common: app still initializing, dependent service not ready, health check URL wrong in probe config." },
  { q: "kubectl logs shows 'OOMKilled' — how to prevent recurrence?", opts: ["Disable memory limits", "Use more nodes", "Set appropriate memory requests/limits, use VPA, analyze heap dumps to find leak", "Reduce the application"], ans: 2, exp: "Fix OOMKilled: 1) Increase limits.memory if app legitimately needs more, 2) Investigate memory leak (heap dump with jmap, memory profiler), 3) Use VPA (VerticalPodAutoscaler) to auto-tune requests/limits, 4) Set JVM -Xmx below container limit (leave 20-30% headroom for JVM overhead)." },
  { q: "Node shows 'NotReady' — what do you check?", opts: ["Service endpoints", "kubectl describe node: Conditions (DiskPressure, MemoryPressure, PIDPressure), kubelet status, network plugin health", "etcd logs", "Pod logs"], ans: 1, exp: "kubectl describe node → Conditions section. Common causes: 1) kubelet not running (systemctl status kubelet), 2) DiskPressure (clean up logs/images: docker system prune), 3) Network plugin (CNI pod crashed in kube-system), 4) Clock skew (kubelet cert expired)." },
  { q: "How to debug a pod that crashes before you can exec into it?", opts: ["Use kubectl attach", "Delete and recreate", "kubectl logs <pod> --previous for last container's logs; use initContainer for debugging; override command with sleep infinity", "You can't"], ans: 2, exp: "Strategies: 1) kubectl logs --previous: logs from CRASHED container. 2) Override command: kubectl run debug --image=same-image -- sleep 3600 then exec in. 3) kubectl debug (ephemeral container). 4) Copy pod spec, add command:['sleep','3600'] to override entrypoint." },
  { q: "Horizontal Pod Autoscaler not scaling — what to check?", opts: ["metrics-server installed and running, resource requests set on pods (HPA needs requests to calculate utilization %)", "Node count", "Service type", "Storage class"], ans: 0, exp: "HPA requirements: 1) metrics-server must be running (kubectl top pods must work), 2) Pods must have resource requests set (HPA calculates utilization% = actual/request), 3) HPA targetCPUUtilizationPercentage threshold actually being reached. kubectl describe hpa shows current metrics." },
  { q: "What does kubectl rollout undo deployment/<name> do?", opts: ["Scales to zero", "Rolls back to the previous ReplicaSet (previous deployment revision)", "Restarts all pods", "Deletes the deployment"], ans: 1, exp: "Kubernetes keeps revision history (default: 10 revisions). rollout undo goes back to previous revision. rollout undo --to-revision=3 for specific version. Check history: kubectl rollout history deployment/my-app. Each rollout creates a new ReplicaSet — undo just shifts the active one." }
])}`;
...
${quizHTML('k8s-advanced', [
  { q: "What is a Kubernetes Operator?", opts: ["A cloud provider plugin", "A Helm chart", "CRD + Controller that codifies operational knowledge — watches CR events and reconciles actual to desired state", "A K8s admin role"], ans: 2, exp: "Operator pattern: 1) CRD defines new resource type (e.g., PostgresCluster), 2) Controller watches for CR creates/updates/deletes, 3) Controller reconciles: creates StatefulSets, Services, sets up replication. Encodes operational expertise: auto-failover, backup, scaling." },
  { q: "Istio sidecar injection — how does it work?", opts: ["DaemonSet on each node", "Custom kubelet plugin", "MutatingWebhookConfiguration intercepts Pod creation, automatically injects Envoy sidecar container", "Manual container addition"], ans: 2, exp: "Istio's mutating admission webhook: when pod is created in a labeled namespace (istio-injection=enabled), Kubernetes API calls Istio's webhook before creating pod. Webhook adds envoy sidecar + init container (sets iptables rules to intercept all traffic). Transparent to app." },
  { q: "What is the purpose of Helm in Kubernetes?", opts: ["Service mesh", "Monitoring tool", "Package manager — bundles K8s manifests as Charts with templating, versioning, and lifecycle management (install/upgrade/rollback)", "Container runtime"], ans: 2, exp: "Helm: K8s package manager. Chart = collection of K8s templates + values.yaml. helm install: renders templates with values, applies to cluster. helm upgrade --atomic: apply + rollback on failure. Repository: share reusable charts (Bitnami, official). 3-way merge for upgrades." },
  { q: "PodDisruptionBudget (PDB) prevents?", opts: ["Horizontal scaling", "Pod resource overuse", "Involuntary disruptions (node crash)", "Too many pods being voluntarily disrupted simultaneously during node drains or rolling updates"], ans: 3, exp: "PDB: minAvailable=2 means at least 2 pods must be available during disruptions. kubectl drain respects PDB — will wait if draining would violate it. Critical for HA: ensures rolling updates and node maintenance don't take down more than allowed fraction of your service." },
  { q: "Resource Requests vs Limits — what happens when a container exceeds CPU limit?", opts: ["Pod is evicted", "Nothing — limits are suggestions", "CPU is throttled (container gets less CPU time), not killed — unlike memory", "Container is killed"], ans: 2, exp: "CPU limit: container gets throttled (CFS bandwidth control) — runs slower but not killed. Memory limit: container is OOMKilled if it exceeds. This is why memory leaks are more dangerous than CPU spikes. Always set requests (scheduler uses for placement) and limits (cgroup enforcement)." },
  { q: "What is RBAC in Kubernetes?", opts: ["Network policy", "Image scanning", "Container security context", "Role-Based Access Control — ServiceAccounts + Roles/ClusterRoles + RoleBindings control what can access what in the cluster"], ans: 3, exp: "RBAC: ServiceAccount → RoleBinding → Role (namespace-scoped) or ClusterRoleBinding → ClusterRole (cluster-wide). Role defines: verbs (get, list, create, delete) on resources (pods, secrets, deployments). Principle of least privilege: give pods only permissions they need." },
  { q: "What triggers a Horizontal Pod Autoscaler to scale up?", opts: ["Current metric (CPU/memory/custom) exceeds target threshold — HPA calculates desired replicas = current × (current/target)", "Manual trigger only", "Time of day", "Node addition"], ans: 0, exp: "HPA formula: desiredReplicas = ceil(currentReplicas × (currentMetricValue / desiredMetricValue)). If 2 pods at 90% CPU, target=50%: desiredReplicas = ceil(2 × 90/50) = ceil(3.6) = 4 pods. Requires metrics-server (CPU/memory) or custom metrics adapter (Prometheus adapter for business metrics)." },
  { q: "What is a Kubernetes InitContainer?", opts: ["Container that runs to completion before app containers start — used for setup, waiting for dependencies, config generation", "A lightweight main container", "A container with reduced permissions", "A sidecar that runs forever"], ans: 0, exp: "InitContainers run sequentially before main containers. Uses: 1) Wait for DB to be ready (init: check DB connection loop), 2) Download config from vault, 3) Run database migrations, 4) Set file permissions. If init container fails, pod restarts. Main containers only start when all inits succeed." },
  { q: "Network Policy in Kubernetes — default behaviour without any policy?", opts: ["Only Service traffic allowed", "Only same-namespace allowed", "All traffic blocked", "All traffic allowed — pods can communicate with all other pods by default"], ans: 3, exp: "Default K8s: no NetworkPolicy = all pods can talk to all pods (flat network). Add a NetworkPolicy → only matching traffic allowed. Implement zero-trust: deny all by default, then allow specific ingress/egress per namespace/pod selector. Requires CNI support (Calico, Cilium, Weave)." }
])}`;
...
${quizHTML('k8s-observability', [
  { q: "Three pillars of observability?", opts: ["Availability Performance Security", "Logs for what happened, Metrics for aggregated numbers, Traces for request journey across services", "Latency Throughput Errors", "CPU Memory Network"], ans: 1, exp: "Logs: timestamped events showing what happened and why. Metrics: numeric aggregates over time such as QPS error rate and CPU percent. Traces: distributed request journey showing which service added latency. All three are needed for complete debugging of distributed systems." },
  { q: "Prometheus pull model advantage?", opts: ["Prometheus controls scrape interval and detects down services immediately since scrape fails; services need no push config", "Push reduces latency", "No real difference", "Push is always better"], ans: 0, exp: "Pull advantages: if a service is down Prometheus knows immediately because the scrape fails and can trigger an alert. Scrape interval is controlled centrally. Services just expose a /metrics endpoint with no push destination configuration needed. Use Pushgateway for batch jobs." },
  { q: "What is a distributed trace span?", opts: ["A log line", "A Prometheus counter", "A metric data point", "A single timed operation within a trace having traceID spanID parentSpanID start and end time and attributes"], ans: 3, exp: "A trace is a tree of spans. Each span represents one operation such as an HTTP call DB query or cache lookup. All spans in a request share the same traceID and are linked by parentSpanID. Together they show the full journey of one request across all services with timing of each step." },
  { q: "PromQL rate() vs irate() when to use each?", opts: ["irate is deprecated", "They are identical", "rate is only for gauges", "rate gives smoothed average over full window best for alerts. irate gives instant rate from last 2 points better for seeing spikes"], ans: 3, exp: "rate(requests[5m]) gives the per-second average over 5 minutes which is smooth and good for alerting since it avoids false triggers from brief spikes. irate() uses only the last 2 data points so it is very sensitive and shows spikes but is noisy. Use rate for alerts and irate for investigation dashboards." },
  { q: "RED method stands for?", opts: ["Reliability Efficiency Durability", "Response Encoding Data", "Rate requests per second, Errors error rate, Duration latency percentiles p50 p95 p99", "Requests Execution Downtime"], ans: 2, exp: "RED is the golden signals framework for measuring service health. Rate: how many requests per second. Errors: what fraction fail. Duration: how long they take measured as p50 p95 and p99. The USE method covers infrastructure resources as Utilization Saturation and Errors. A dashboard with RED covers 90 percent of service incidents." },
  { q: "Structured logging main advantage?", opts: ["Faster to write", "Plain text uses less space", "Machine-parseable JSON enables filtering and alerting by specific fields like userId or traceId", "No practical difference"], ans: 2, exp: "Plain text logs can only be grepped. Structured JSON in Loki and Elasticsearch lets you filter by any field instantly. The key technique is including traceId in every log line so you can correlate log events with distributed traces in Jaeger. This ability to jump between logs and traces is the most powerful debugging approach available." },
  { q: "OpenTelemetry relationship to Prometheus and Jaeger?", opts: ["OTel is vendor-neutral instrumentation standard and Prometheus with Jaeger are backends receiving OTel data via OTLP", "OTel fully replaces both", "They compete directly", "No relationship"], ans: 0, exp: "OpenTelemetry provides a standardised SDK plus collector plus the OTLP wire protocol. Instrument your code once then export to any backend. The OTel Collector fans out to Prometheus for metrics, Jaeger or Tempo for traces, and Loki or Elasticsearch for logs. This separates instrumentation from backend choice eliminating vendor lock-in." },
  { q: "SLI vs SLO vs SLA differences?", opts: ["SLA is only internal", "SLO is the measurement", "SLI is the actual measurement. SLO is the internal target like 99.9 percent. SLA is the contractual commitment with penalties", "All the same"], ans: 2, exp: "SLI indicator: the actual measurement such as percent of requests completing under 200ms. SLO objective: the internal target you set such as 99.9 percent. SLA agreement: the external contract with customers that carries financial penalties if breached. Always set your SLO tighter than your SLA to maintain an error budget buffer." },
  { q: "Error budget practical usage?", opts: ["Maximum errors per day", "The allowed failure quota: spend on risky deployments and freeze changes when exhausted to focus on reliability", "A financial budget for bug fixes", "A cloud cost budget"], ans: 1, exp: "With SLO of 99.9 percent the error budget is 0.1 percent of requests allowed to fail which equals about 43 minutes of downtime per month. Fast teams burn this budget with risky deployments. When the budget runs out the team declares a deployment freeze and focuses on reliability improvements making reliability a shared product and engineering responsibility." }
])}`;
...
${quizHTML('distributed', [
  { q: "CAP: in presence of partition, what can't you have both of?", opts: ["Consistency + Availability — you must choose one", "Availability + Partition tolerance", "Consistency + Partition tolerance", "Consistency + Availability"], ans: 0, exp: "Network partitions always happen (hardware fails, cables cut). CAP: choose CP (refuse requests to stay consistent — etcd, ZooKeeper) or AP (serve stale data to stay available — Cassandra, DynamoDB). CA systems only work on single machines (no partition tolerance possible in distributed systems)." },
  { q: "PACELC extends CAP — what does the ELC part mean?", opts: ["External Load Control", "Eventually Linked Clusters", "Error-Latency-Correctness", "Else: even without partition, trade-off between Latency and Consistency exists"], ans: 3, exp: "PACELC: if Partition → choose A or C. Else (no partition) → choose L (low latency) or C (consistency). Example: DynamoDB default = EL (fast reads, eventual consistency). With strongly consistent reads = EC (higher latency, consistent). Captures real-world tradeoffs better than CAP." },
  { q: "Why is Kafka much faster than traditional message queues?", opts: ["Uses in-memory storage only", "Single-threaded design", "Sequential disk writes + OS page cache + zero-copy sendfile + batch compression", "Uses UDP"], ans: 2, exp: "1) Sequential writes to disk (OS page cache makes this near-RAM speed), 2) sendfile() zero-copy syscall (data goes disk→network without user-space copy), 3) Producer batching + compression (LZ4/Snappy/ZSTD), 4) No per-message state — just an offset. RabbitMQ maintains per-message ACK state." },
  { q: "Saga pattern vs 2PC — why use Saga?", opts: ["2PC is deprecated", "Saga is faster for small transactions", "2PC holds distributed locks during protocol — slow, blocking, single point of failure. Saga: local transactions + compensating actions, no cross-service locks", "Saga is more consistent"], ans: 2, exp: "2PC: Phase 1 prepare (all participants lock resources and vote), Phase 2 commit/abort. Problem: coordinator failure during phase 2 = resources locked indefinitely. Saga: each step local transaction, failure triggers compensating transactions in reverse. No locks, more available." },
  { q: "Raft consensus — when is a write committed?", opts: ["When leader writes it", "When all nodes acknowledge", "When leader sends heartbeat", "When majority of nodes (N/2 + 1) have written the log entry to their local logs"], ans: 3, exp: "Raft write: client → leader appends to log → sends AppendEntries to all followers → once majority (N/2+1) confirm → leader commits → responds to client → followers commit on next heartbeat. Majority quorum ensures durability even if minority of nodes fail." },
  { q: "Eventual consistency — what does it guarantee?", opts: ["Strong ordering of all operations", "Reads always see latest write", "Given no new writes, all replicas will eventually converge to the same value", "Nothing"], ans: 2, exp: "Eventual consistency: replicas may temporarily diverge (read stale data), but if you stop writing, all replicas will eventually agree. Does NOT guarantee: when convergence happens, ordering of concurrent writes. Cassandra, DynamoDB, DNS — all eventually consistent by default." },
  { q: "Consistent hashing — what problem does it solve?", opts: ["Data encryption", "Load balancing", "Replication ordering", "When adding/removing cache/DB nodes: only ~1/N keys rehash (vs all keys with naive hash % N)"], ans: 3, exp: "Naive sharding (key % N): adding 1 node changes N, almost all keys hash to different node = cache miss storm. Consistent hashing: keys and nodes on a ring. Adding a node: only keys between it and its predecessor change. Used by Cassandra, DynamoDB, CDNs, Memcached." },
  { q: "What is idempotency key and why is it essential for payments?", opts: ["A hash of the payment amount", "A unique request ID — allows safely retrying failed requests; server detects duplicate and returns cached result", "A session token", "An encryption key"], ans: 1, exp: "Payment timeout: did it succeed or not? Retry without idempotency = double charge. Idempotency key: client generates UUID, sends with request. Server: if key seen before, return cached result. If not, process and store result with key. Stripe, PayPal all require idempotency keys for payments." },
  { q: "Leader election — what guarantees does ZooKeeper provide?", opts: ["Strong consistency via ZAB protocol (Zookeeper Atomic Broadcast) — linearizable reads from leader, sequential consistency", "Only availability", "Eventual consistency", "No consistency guarantees"], ans: 0, exp: "ZooKeeper uses ZAB (Paxos-like). Writes: always go through leader, applied in total order. Reads from leader: linearizable (see all committed writes). Ephemeral znodes: auto-deleted when client session ends — used for leader election (first to create wins, others watch for deletion)." }
])}`;
...
${quizHTML('k8s-gpu', [
  { q: "What does the NVIDIA Device Plugin do in Kubernetes?", opts: ["Monitors GPU temperature", "Enables CUDA in containers", "Exposes GPUs as schedulable K8s resources (nvidia.com/gpu: 8) so the scheduler can assign GPUs to pods", "Installs GPU drivers"], ans: 2, exp: "Without the device plugin, Kubernetes doesn't know GPUs exist. The plugin registers each GPU as a resource, enables pods to request specific GPU counts via resource limits, and manages GPU allocation to prevent double-booking." },
  { q: "Tensor Parallelism vs Pipeline Parallelism — key difference?", opts: ["TP is faster always", "PP is only for CPUs", "No difference", "TP splits each layer vertically across GPUs within a node (microsecond comms). PP splits layers sequentially across nodes (tolerates ms latency)"], ans: 3, exp: "TP: GPU 0 and GPU 1 each compute half of every layer simultaneously — requires synchronization after every layer (high frequency, must be fast, PCIe/NVLink). PP: GPU 0 does layers 1-48, GPU 1 does 49-96 — only one transfer per stage per token (tolerates network latency)." },
  { q: "Why use 16 GPUs instead of 8 for a single large model?", opts: ["Required by Kubernetes", "16 GPUs cheaper than 8", "Each GPU holds less model weight, freeing more GPU memory for KV cache → 4x more concurrent users", "16 GPUs = 2x faster always"], ans: 2, exp: "With 8 GPUs: 93.7 GB model per GPU, only 32 GB for KV cache (~25 concurrent users at 64K context). With 16 GPUs: 46.9 GB model per GPU, 73 GB for KV cache (~98 concurrent users). Nearly 4x concurrency improvement by spreading model across more GPUs." }
,
  { q: "What is CUDA and why does it matter for ML workloads?", opts: ["A database format", "A container runtime", "NVIDIA's parallel computing platform — GPU cores execute thousands of threads simultaneously, ideal for matrix math in deep learning", "A network protocol"], ans: 2, exp: "CUDA (Compute Unified Device Architecture): NVIDIA's programming model for GPU acceleration. A GPU has thousands of cores (H100: 16,896 CUDA cores) vs CPU's ~64. Matrix multiplication (core of neural net forward/backward pass) maps perfectly to SIMD GPU parallelism. cuDNN and cuBLAS are optimised CUDA libraries used by PyTorch/TensorFlow." },
  { q: "What is CDI (Container Device Interface)?", opts: ["A container image format", "A K8s storage driver", "A CPU affinity tool", "A standard for container runtimes to access devices (GPUs) — replaces vendor-specific hooks with a common spec"], ans: 3, exp: "CDI: Container Device Interface standard (co-developed by NVIDIA). Before CDI, each container runtime needed custom hooks to inject GPU drivers. CDI defines a standard JSON spec describing all files/devices/envvars needed for a device. Container runtime reads CDI spec and injects device without vendor-specific code." },
  { q: "What is DRA (Dynamic Resource Allocation) in Kubernetes?", opts: ["A dynamic autoscaling algorithm", "A DNS resolution approach", "A disk management system", "A K8s 1.26+ API replacing device plugin model — allows fine-grained resource requests with parameters and structured sharing"], ans: 3, exp: "DRA (Dynamic Resource Allocation): replaces device plugin model. Key improvements: 1) Resources can have parameters (request specific GPU model), 2) Resources can be shared between pods (not just exclusive), 3) Vendor drivers (ResourceClaim drivers) handle allocation. Enables advanced GPU partitioning like MIG (Multi-Instance GPU) in K8s." },
  { q: "NVIDIA MIG (Multi-Instance GPU) — what problem does it solve?", opts: ["Network bandwidth", "Memory compression", "A large GPU can be partitioned into smaller isolated instances — different pods/users get dedicated GPU slices without interference", "Driver isolation"], ans: 2, exp: "MIG partitions an H100 GPU into up to 7 isolated GPU instances (e.g., 1g.10gb = 1/7 of the GPU with 10GB memory). Each instance has guaranteed memory, compute, and bandwidth. Run 7 smaller ML workloads on 1 GPU instead of wasting 500GB GPU on a 10GB inference job. GPU Operator automates MIG configuration in Kubernetes." },
  { q: "PagedAttention in vLLM solves what GPU memory problem?", opts: ["Driver compatibility", "KV cache fragmentation — uses virtual memory paging so different requests share GPU memory efficiently without waste", "Multi-node networking", "Slow startup time"], ans: 1, exp: "KV cache (key-value attention cache): each token in context needs stored. Traditional: allocate max_seq_len upfront (wastes memory for short requests). PagedAttention: allocates in pages (like OS virtual memory). Short requests use small pages, long requests get more. 3-4x better memory utilisation → more concurrent users." },
  { q: "What is NCCL and when does it activate in multi-GPU inference?", opts: ["A checkpoint format", "A network card driver", "NVIDIA's collective communications library — handles AllReduce, AllGather between GPUs during tensor parallel operations", "A scheduling algorithm"], ans: 2, exp: "NCCL: optimised GPU-to-GPU communication. Used during tensor parallelism: after each GPU processes its weight slice, AllReduce combines results across all GPUs before moving to next layer. Intra-node: P2P/IPC over PCIe/NVLink. Inter-node: GPU Direct RDMA over InfiniBand/RoCE — CPU bypassed entirely." }
])}
`;
